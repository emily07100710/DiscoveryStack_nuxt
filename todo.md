...
- [x] 實作來源政策檢查、速率限制、HTML-to-text 清洗、去重、PII 偵測／遮罩與人工審核隊列，禁止把未經核准的公開頁面直接送入訓練集。
- [x] 以至少一個明確允許再利用的公開資料來源，端到端驗證來源登錄、多維標註、人工審核、manifest lineage 與 training gate；不將未授權來源提交至 Hugging Face。
...
- [x] 建立至少 100 筆去重、具來源／授權依據、完成 PII 檢查、可追溯與人工核准的公開 SEO／GEO 訓練資料；不得以樣板、隨機內容、未核實授權或重複資料湊數。
- [x] 對該 100 筆資料建立不可變的 dataset manifest，顯示來源摘要、授權狀態、資料分割、標籤分布、去重結果與品質／PII 審核結果。
...
- [x] 執行 Google Search Central batch-30 的 structured-data testing、preferred sources、Article、ProfilePage、DiscussionForumPosting 與 QAPage 文件受控收集，逐筆依 PII 結果完成或保留品質審核與多維人工標註。
