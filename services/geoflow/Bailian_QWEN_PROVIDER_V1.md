...
The workspace label is restricted to lowercase ASCII letters, digits, and interior hyphens. URLs must use HTTPS, have no userinfo, query, or fragment, use the default HTTPS port, contain no IP literal, and have the exact `/compatible-mode/v1` path or that path followed only by `/chat/completions` for server canonicalization. Encoded slash, double-encoded path, traversal, backslash, arbitrary compatible hosts, and hostname suffix/prefix spoofing fail closed. The server constructs `/chat/completions`; callers do not choose a complete request URL. No cross-origin redirect is enabled by the existing secure outbound boundary.
