...
<template>
  <section class="lead-admin" aria-labelledby="lead-admin-title">
    <header class="lead-admin-head">
      ...
      <nav aria-label="私有工具"><NuxtLink to="/audit-lab">稽核實驗室</NuxtLink><NuxtLink to="/ml-lab-preview">ML 工作台</NuxtLink></nav>
    </header>
      ...
      <div v-if="leads.length" class="lead-list">
        <article v-for="lead in leads" :key="lead.id">
          ...
          <dl>
            ...
            <div><dt>模型改善</dt><dd>{{ lead.modelImprovementConsent ? '已另外同意' : '未同意' }}</dd></div>
          </dl>
