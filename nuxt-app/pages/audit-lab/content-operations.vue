...
type PublicationTarget = { id: string | number, clientId: string | number, targetId: string, websiteId?: string | null, framework: Framework | string, transport: PublicationTransport | string, targetOrigin: string, contentRoot: string, defaultBranch: string | null, status: string, activeSlot?: number | null, executionEnabled: boolean, credentialConfigured: boolean, destinationPublicationIdentityConfigured?: boolean, serviceReferenceConfigured?: boolean, repositoryOwner?: string | null, repositoryName?: string | null, endpointPath?: string | null, allowedContentTypes?: string[], allowedLanguages?: string[] }
...
type AutopilotPolicy = { policyId: string, status: string, expiresAt: string, revokedAt?: string | null, allowedContentTypes: string[], allowedLanguages: string[], targetId: string, targetRowId?: string | number, maximumRiskLevel?: string, configurationFingerprint: string }
...
const autopilotPolicyFor = (clientId: string | number) => autopilotPolicyViewFor(clientId).policy
const autopilotPoliciesFor = (clientId: string | number) => autopilotPolicyViewFor(clientId).policies
const autopilotActivePoliciesFor = (clientId: string | number) => autopilotPoliciesFor(clientId).filter(policy => ['enabled', 'paused'].includes(policy.status))
const autopilotStatusFor = (clientId: string | number) => autopilotActivePoliciesFor(clientId)[0]?.status || autopilotPoliciesFor(clientId)[0]?.status || 'not_configured'
...
const targetForm = reactive<{ clientId: string, framework: Framework, transport: PublicationTransport, targetOrigin: string, contentRoot: string, defaultBranch: string, credentialReference: string, repositoryOwner: string, repositoryName: string, endpointPath: string, serviceReference: string, maximumPayloadBytes: number, executionEnabled: boolean }>({ clientId: '', framework: 'astro', transport: 'first_party_git', targetOrigin: '', contentRoot: 'content', defaultBranch: 'main', credentialReference: '', repositoryOwner: '', repositoryName: '', endpointPath: '/api/content-ingest', serviceReference: '', maximumPayloadBytes: 1000000, executionEnabled: false })
...
const capabilityItems = computed(() => [
  ...
  { key: 'outcomeCollectionConfigured', label: '內部 outcome collection', falseMessage: 'Outcome persistence 尚未可用', trueMessage: 'Outcome assessment persistence 已可用', available: workspace.value.capabilities.outcomeCollectionConfigured },
])
...
const entryClientId = (entry: ContentEntry) => workspace.value.calendars.find(calendar => String(calendar.id) === String(entry.calendarId))?.clientId
const entryTargetsFor = (entry: ContentEntry) => workspace.value.publicationTargets.filter(target => String(target.clientId) === String(entryClientId(entry)) && (target.status === 'active' || (entry.publicationTargetBindings || []).some(binding => String(binding.targetRowId) === String(target.id))))
const entryBindingSelection = reactive<Record<string, Array<string | number>>>({})
const entryBindingSelectionFor = (entry: ContentEntry) => {
  const key = String(entry.id)
  if (!entryBindingSelection[key]) entryBindingSelection[key] = (entry.publicationTargetBindings || []).map(binding => binding.targetRowId)
  return entryBindingSelection[key]!
}
const isEntryTargetSelected = (entry: ContentEntry, targetId: string | number) => entryBindingSelectionFor(entry).some(value => String(value) === String(targetId))
function toggleEntryBinding(entry: ContentEntry, targetId: string | number, event: Event) {
  const checked = (event.target as HTMLInputElement).checked
  const selected = entryBindingSelectionFor(entry)
  const index = selected.findIndex(value => String(value) === String(targetId))
  if (checked && index === -1) selected.push(targetId)
  if (!checked && index >= 0) selected.splice(index, 1)
}
async function bindEntryTargets(entry: ContentEntry) {
  const selected = entryBindingSelectionFor(entry)
  await post(`/api/content-operations/entries/${entry.id}/publication-targets`, { targetRowIds: selected.map(Number) }, 'Entry publication target bindings 已更新；畫面正在重新整理。')
}
...
<template>
  <main class="operations-page">
    ...
    <template v-else>
      ...
      <section class="section-block" aria-labelledby="v4-runtime-title"><div class="section-heading"><div><p class="eyebrow">V4 RUNTIME GOVERNANCE</p><h2 id="v4-runtime-title">Durable authority 與逐 target 收據</h2></div><span class="data-note">private · noindex · advisory only</span></div><p class="section-copy">此處只顯示持久化治理事實，不宣稱 GEO 成效。Mode、entity/query authority、budget、repair、replacement、machine authorization 與 receipt 都由 server repository 投影；hard safety 不因 mode 改變。</p><div class="runtime-matrix"><span><strong>{{ workspace.governance.policies.length }}</strong> V3/V4 policies</span><span><strong>{{ workspace.governance.entityProfiles.length }}</strong> entity profiles</span><span><strong>{{ workspace.governance.queryOwnership.length }}</strong> query owners</span><span><strong>{{ workspace.governance.budgetReservations.length }}</strong> atomic budget reservations</span><span><strong>{{ workspace.governance.repairs.length }}</strong> durable repairs</span><span><strong>{{ workspace.governance.substitutions.length }}</strong> replacement lineages</span><span><strong>{{ workspace.governance.machineAuthorizations.length }}</strong> machine authorizations</span><span><strong>{{ workspace.entries.flatMap(entry => entry.publicationTargetBindings || []).filter(binding => binding.latestAttempt?.receiptFingerprint).length }}</strong> verified target receipts</span></div></section>
...
.runtime-matrix{display:grid;grid-template-columns:repeat(4,1fr);gap:.55rem;margin-top:1rem}.runtime-matrix span{padding:.65rem;background:#fff;border:1px solid #dce6dc;color:#52655a;font-size:.72rem}.policy-target-list,.binding-options,.binding-receipts{display:grid;gap:.55rem}.policy-target-row{display:grid;grid-template-columns:1fr auto;gap:.25rem .6rem;padding:.65rem;background:#f7faf7;border:1px solid #dce6dc}.policy-target-row small{grid-column:1/-1;color:var(--muted);font-size:.72rem}.entry-bindings{display:grid;gap:.75rem;margin-top:1rem;padding:1rem;background:#f2f7f2;border:1px solid #d5e0d5}.entry-bindings__heading{display:flex;align-items:center;justify-content:space-between;gap:1rem}.entry-bindings__heading div{display:grid;gap:.25rem}.entry-bindings__heading span{color:var(--muted);font-size:.72rem}.binding-option{display:flex!important;grid-template-columns:none!important;align-items:flex-start;gap:.55rem;padding:.7rem;background:#fff;border:1px solid #dce6dc}.binding-option input{width:auto!important;margin-top:.2rem}.binding-option span{display:grid;gap:.25rem;color:var(--ink)}.binding-option small,.binding-receipt small{color:var(--muted);font-size:.72rem}.binding-receipt{display:grid;grid-template-columns:1fr auto;gap:.3rem .7rem;padding:.7rem;background:#fff;border:1px solid #dce6dc;font-size:.76rem}.binding-receipt small{grid-column:1/-1;word-break:break-all}.target-list{display:grid;gap:.55rem}.target-list div{padding:.7rem;background:#fff;border:1px solid #dce6dc}.target-list dd{margin:.3rem 0 0;color:var(--muted);font-size:.75rem;line-height:1.5;word-break:break-word}@media(max-width:900px){.runtime-matrix{grid-template-columns:repeat(2,1fr)}}@media(max-width:620px){.runtime-matrix{grid-template-columns:1fr}.entry-bindings__heading{display:grid;align-items:start}.binding-receipt{grid-template-columns:1fr}}
</style>
