...
const guidedStrategies = ref<Array<{ id: number, issueCode: string, recommendationKey: string, priority: string, rationale: string, ruleIds: string[], contentOpportunities: unknown[] }>>([])
const guidedPlan = ref<{ plan: { id: number, status: string }, deliverables: Array<{ id: number, title: string, status: string }> } | null>(null)
...
async function guidedDiagnose() {
  const result = await post<{ diagnosisId: number, strategyRecommendations: Array<{ id: number, issueCode: string, recommendationKey: string, priority: string, rationale: string, ruleIds: unknown, contentOpportunities: unknown }> }>('/api/seo-geo/diagnose', { homepageUrl: guided.homepageUrl, sourceId: numberOrUndefined(guided.sourceId) }, 'Step 1 完成：Diagnosis 與可選 Strategy recommendations 已建立。')
  if (result) {
    guided.diagnosisId = String(result.diagnosisId)
    guidedStrategies.value = result.strategyRecommendations.map(strategy => ({ ...strategy, ruleIds: Array.isArray(strategy.ruleIds) ? strategy.ruleIds.filter((item): item is string => typeof item === 'string') : [], contentOpportunities: Array.isArray(strategy.contentOpportunities) ? strategy.contentOpportunities : [] }))
    guided.selectedStrategyIds = guidedStrategies.value.map(strategy => strategy.id)
    guided.planId = ''
...
async function guidedCreatePlan() {
  ...
  const result = await post<{ plan: { id: number, status: string }, deliverables: Array<{ id: number, title: string, status: string }> }>('/api/seo-geo/production-plans', { diagnosisId: Number(guided.diagnosisId), strategyRecommendationIds: guided.selectedStrategyIds, title: guided.planTitle.trim() || `SEO/GEO production plan · ${guidedStrategies.value[0]?.recommendationKey || 'selected strategy'}`, language: guided.language, idempotencyKey: idempotencyKey() }, 'Step 2 完成：Production Plan 與 bounded deliverables 已建立。')
  if (result) {
    guided.planId = String(result.plan.id)
    guidedPlan.value = result
  }
}
async function guidedGeneratePlan() {
  if (!guided.planId) return
  const result = await post<{ plan: { id: number, status: string }, deliverables: Array<{ id: number, title: string, status: string }> }>(`/api/seo-geo/production-plans/${guided.planId}/generate`, {}, 'Step 3 完成：Brief／Job／candidate generation 已依序處理；仍須人工 review，沒有發布。')
  if (result) guidedPlan.value = result
}
function saveBrief() {
