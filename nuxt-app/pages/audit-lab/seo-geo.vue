...
type Workspace = {
  ...
  evidenceApprovals: Array<{ id: number, sourceId: number, artifactId: number | null, sourceName?: string | null, sourceUrl?: string | null, fallbackSourceUrl?: string | null, artifactType?: string | null, artifactHash?: string | null, allowedFor: string, status: string, approvedAt: string | null }>
...
type GuidedDeliverable = { id: number, title: string, contentType: string, audience?: string, status: string, brief?: any, job?: any, baseDraft?: any, optimizedDraft?: any, riskGate?: any, reviews?: any[], previews?: any[], eligibility?: { canReview: boolean, canApprovePreview: boolean, canApproveDelivery: boolean, canPreview: boolean, canExport: boolean } }
...
const guidedSources = computed(() => [...new Set(workspace.value.evidenceApprovals.filter(entry => entry.status === 'approved' && entry.allowedFor === 'diagnosis' && !entry.artifactId).map(entry => entry.sourceId))])
const guidedEvidenceOptions = computed(() => workspace.value.evidenceApprovals.filter(entry => entry.status === 'approved' && entry.allowedFor === 'diagnosis' && !entry.artifactId))
const revisionForms = reactive<Record<number, { title: string, body: string }>>({})
function revisionFormFor(deliverableId: number) { return revisionForms[deliverableId] || (revisionForms[deliverableId] = { title: '', body: '' }) }
...
async function guidedRevision(deliverable: GuidedDeliverable) {
  const form = revisionFormFor(deliverable.id)
  if (!guided.planId || !form.title.trim() || form.body.trim().length < 40) { actionError.value = '請填寫新的 revision title 與至少 40 字正文。'; return }
  const result = await post<{ detail?: GuidedPlanDetail }>(`/api/seo-geo/production-plans/${guided.planId}/revisions/${deliverable.id}`, { title: form.title, body: form.body }, '新 revision 已重新執行 risk gate；請依狀態再次進行 owner review。')
  if (result?.detail) guidedPlan.value = result.detail
}
async function guidedExport(deliverable: GuidedDeliverable, format: 'markdown' | 'json') {
  if (!guided.planId || !deliverable.id) return
  actionState.value = 'saving'; actionNotice.value = ''; actionError.value = ''
  try {
    const response = await globalThis.fetch(`/api/seo-geo/production-plans/${guided.planId}/export/${deliverable.id}`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ format }) })
    if (!response.ok) throw new Error((await response.text()) || 'Export 未完成。')
    const blob = await response.blob()
    const filename = response.headers.get('content-disposition')?.match(/filename="([^"]+)"/i)?.[1] || `production-${deliverable.id}.${format === 'markdown' ? 'md' : 'json'}`
    const url = URL.createObjectURL(blob)
    const anchor = document.createElement('a'); anchor.href = url; anchor.download = filename; anchor.click(); URL.revokeObjectURL(url)
    actionNotice.value = 'Export 已記錄；這是本地 owner response，沒有外部 delivery write。'
    await refreshGuidedPlan()
  } catch (error: any) { actionError.value = error?.message || 'Export 未完成；沒有發出外部 delivery write。' } finally { actionState.value = 'idle' }
}
