...
const guidedEvidenceOptions = computed(() => workspace.value.evidenceApprovals.filter(entry => entry.status === 'approved' && ['diagnosis', 'recommendation', 'content_draft'].includes(entry.allowedFor) && (entry.allowedFor === 'diagnosis' || entry.artifactId)))
...
async function guidedRevision(deliverable: GuidedDeliverable) {
  ...
  const result = await post<{ detail?: GuidedPlanDetail }>(`/api/seo-geo/production-plans/${guided.planId}/revisions/${deliverable.id}`, { title: form.title, body: form.body }, '新 revision 已重新通過 risk gate；請再次進行 owner review。')
