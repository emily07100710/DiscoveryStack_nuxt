...
The implementation is intended for review after the repository's required frozen install, typecheck, targeted tests, three-engine regression, SEO/GEO regression, production build, diff checks, restricted scans, secret scans, and artifact scans. Full Vitest, migration execution, production migration, external provider calls, external content writes, and deployment remain intentionally out of scope.
