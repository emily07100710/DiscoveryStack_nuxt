...
空 history 只允許建立 attempt 1。非空 history 中，只有 `retry_wait` 記錄可以銜接下一個 attempt，而且下一個 attempt 的 `occurredAt` 不得早於前一筆 persisted `retryEligibleAt`；任何較早的 terminal 或 in-flight 記錄都不能被後續偽造記錄掩蓋。最後狀態只有 `retry_wait` 才能建立下一次 command；最後狀態為 `scheduled`、`eligible`、`dispatch_planned`、任何 terminal state 或 `cancelled` 都不能直接建立下一次 command。`dispatch_planned` 明確回 `ATTEMPT_STILL_IN_FLIGHT`，terminal state 回 `TERMINAL_STATE`。
...
所有 timezone-bearing timestamps 都先經過 strict calendar parser：格式、month/day、leap year、hour/minute/second 與 offset component 必須逐項合法，不存在的日期、leap second 與 malformed offset 都拒絕；通過後才 canonicalize 成 UTC ISO。Success time gate 強制 `persisted dispatch occurredAt <= attemptStartedAt <= publishedAt <= now`，並回傳專用 `PUBLISHED_AT_BEFORE_ATTEMPT`、`PUBLISHED_AT_IN_FUTURE` 或 persisted attempt evidence error。target origin 會重新經過 target validator；remote URL 必須是 HTTPS、無 userinfo/query/fragment、無不允許的 port、無 private/special-use host，且 normalized origin 必須與 target origin 完全相同。
