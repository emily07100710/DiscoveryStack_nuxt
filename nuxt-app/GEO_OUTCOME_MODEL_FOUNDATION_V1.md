...
The following remain `NOT RUN` or intentionally blocked because the requested safety policy forbids them or because credentials/data are unavailable: production migration application; runtime validation against a real disposable MySQL database (the strict transactional Drizzle harness validates repository expressions, owner scope, constraints, concurrency and rollback, but is explicitly **not** proof of real MySQL migrations or runtime behavior); real provider calls; real website crawling; browser proxying; Colab; Qwen/OpenAI/Gemini/Claude calls; external dataset downloads; Hugging Face/model repository uploads; production model promotion; customer-site writes; DNS/payment operations; and claims of real-world citation probability or business uplift.
