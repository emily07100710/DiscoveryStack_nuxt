...
`buildRetrievalResult()` 接受完整 normalized content input、strict candidate records 與必要的 retrieval context，從 `topic + workingTitle + primaryQuestion + audience + goals + language` 產生 Unicode-normalized lexical query tokens。English 以 deterministic word tokens 正規化；CJK 以 deterministic code-point/bigram tokenization。每個 candidate 計算 bounded overlap count/ratio，zero relevance 不進結果，依 relevance 再以 stable code-unit identity tie-break，最後套用 topK。一般 caller candidate API 仍會 strict-normalize candidate，但 **canonical V1 corpus 不由 caller candidates 決定**。`ContentQualityInput.approvedEvidenceChunks` 是唯一完整、已核准的 retrieval corpus；server canonical call 固定為 `buildRetrievalResult(input, input.approvedEvidenceChunks.map(chunk => ({ chunk })))`。

Result 對每個選中 chunk 保存 `matchedTokenCount`、`queryTokenCount`、`relevanceRatio`、固定 `scoreBasis = deterministic_lexical_overlap_v1` 與 limitations。沒有可接受 chunk 時只回傳 `status: not_ready`，不建立 generic knowledge fallback。canonical ready result 的 per-chunk `limitations` 固定為空陣列，top-level limitations 也只由 server deterministic result 產生；caller 不得加入、刪除、重排或修改任何 limitations。`buildRetrievalResult()` 收到非空 caller chunk limitations 時會直接 blocked；`verifyRetrievalResult()` 會先 strict-normalize presented result，再只從完整 normalized `input.approvedEvidenceChunks` 建立 canonical result，最後對 status、版本、fingerprints、snapshot、chunk identity/order/text/hash/capturedAt、所有 metrics、scoreBasis、reasonCodes、top-level limitations 與 per-chunk limitations 做 canonical exact comparison。省略高相關 evidence、truncated topK、替換 winner、漏掉 tie-break winner、fake metrics/order/fingerprint 或 caller limitations 都 fail closed。
...
`evaluateContentQuality()` 只輸出 `passed`、`needs_human_review` 或 `blocked`，且 `humanReviewRequired` 永遠為 true。對 ready `RetrievalResult`，quality gate 直接呼叫 `verifyRetrievalResult(value, normalizedInput)`；若 verifier 回傳 null，結果必須是 `blocked` 且 reason code 為 `RETRIEVAL_FINGERPRINT_MISMATCH`，不得 fallback 到 presented chunks、subset 或靜默改寫成 canonical。它再驗證 provider output 與 provider body/Markdown exact binding，最後評估 evidence、claim safety、structure、governance、risk、conflict 與 content operational bounds。
...
`tests/geo-content-quality-prompt-rag.test.ts` 使用完全 synthetic fixtures，所有測試直接呼叫正式 public functions，沒有真實網站內容或 provider call。Targeted suite 目前有 **217 direct tests**，保留原有 204 tests，另涵蓋完整 approved corpus canonical selection、omitted higher-ranked evidence、truncated topK、lower-ranked substitution、code-unit tie-break winner omission、exact canonical acceptance、PromptPack/provider/quality-gate subset rejection、caller-injected/deleted/reordered/modified limitations、stable corpus order，以及 malformed Proxy/getter、unknown keys、NaN/Infinity、duplicate identity 等 fail-closed cases。
