...
Result 對每個選中 chunk 保存 `matchedTokenCount`、`queryTokenCount`、`relevanceRatio`、固定 `scoreBasis = deterministic_lexical_overlap_v1` 與 limitations。沒有可接受 chunk 時只回傳 `status: not_ready`，不建立 generic knowledge fallback。canonical ready result 的 per-chunk `limitations` 固定為空陣列，top-level limitations 也只由 server deterministic result 產生；caller 不得加入、刪除、重排或修改任何 limitations。`verifyRetrievalResult()` 會先 strict-normalize presented result，再只從完整 normalized `input.approvedEvidenceChunks` 建立 canonical result，最後對 status、版本、fingerprints、snapshot、chunk identity/order/text/hash/capturedAt、所有 metrics、scoreBasis、reasonCodes、top-level limitations 與 per-chunk limitations 做 canonical exact comparison。省略高相關 evidence、truncated topK、替換 winner、漏掉 tie-break winner、fake metrics/order/fingerprint 或 caller limitations 都 fail closed。
