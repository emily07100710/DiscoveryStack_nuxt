...
The `stripe` payment provider creates Checkout Sessions with native server `fetch` and form encoding. A positive `cadenceDays` is the recurring-plan rule and becomes Stripe `subscription` mode with a daily `interval_count`; zero means one-time `payment` mode. Every line is re-summed before transport. The session, PaymentIntent or Subscription receives the exact signed metadata authority needed by later webhooks, including the pre-call checkout-receipt authority fingerprint. Stripe configuration is verified with exactly one read-only `GET /v1/balance`; verification never creates or mutates a Stripe object.

The webhook routes read raw bytes and verify the signature before obtaining or querying a repository. The Stripe route accepts the `Stripe-Signature` `t`/`v1` contract with a 300-second tolerance and maps paid Checkout Session, PaymentIntent success, full Charge refund, and Charge dispute events. It then validates signed owner/order/release metadata and owner-claimed order/quote lineage. Success, failure, cancellation, full refund, dispute, duplicate, payload collision, and out-of-order events are recorded as reduced append-only receipts. Raw bodies and signatures are never retained. An exact verified success atomically moves the order and release to `payment_verified` and appends one `provisioning_armed` audit marker; it does not enqueue a provisioning attempt. Full refunds and disputes suspend the subscription/project, move the order to `refunded` or `disputed`, and block the release. V1 models a full refund; partial-refund commercial policy remains owner/provider-specific.
...
- Production payment supports the exact registered `internal_hmac_v1` and `stripe` adapters. Stripe remains unusable until its allowlisted origins, opaque API/webhook credential references, server verification, and provider-side webhook registration are configured. No Taiwanese payment provider is implemented or implied. The deployment transport is authenticated bearer, not a signed request/response protocol.
...
- The internal payment, domain registration, DNS/TLS, and existing-site ownership adapters retain their exact HMAC broker runtime contracts and remain unverified until the owner supplies allowlisted origins, opaque credential references, and compatible staging providers. Stripe provider account connectivity and delivery also remain unverified outside the mocked tests in this repository.
