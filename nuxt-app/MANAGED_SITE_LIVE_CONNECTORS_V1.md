...
The webhook routes read raw bytes and verify the signature before obtaining or querying a repository. The Stripe route accepts the `Stripe-Signature` `t`/`v1` contract with a 300-second tolerance and maps paid Checkout Session, PaymentIntent success, full or partial Charge refund, and Charge dispute events. It then validates signed owner/order/release metadata and owner-claimed order/quote lineage. Success, failure, cancellation, refund, dispute, duplicate, payload collision, and out-of-order events are recorded as reduced append-only receipts. Raw bodies and signatures are never retained. An exact verified success atomically moves the order and release to `payment_verified` and appends one `provisioning_armed` audit marker; it does not enqueue a provisioning attempt. Full and partial refunds record the actual amount and use the same terminal `refunded` state. Refunds and disputes suspend the subscription/project and block the release.
