...
afterEach(() => { if (originalOrigins === undefined) delete process.env.DISCOVERYSTACK_MANAGED_SITE_ALLOWED_PROVIDER_ORIGINS; else process.env.DISCOVERYSTACK_MANAGED_SITE_ALLOWED_PROVIDER_ORIGINS = originalOrigins })
...
describe('managed-site reachable server-only provider transports', () => {
  it('performs an owner-triggered bounded Bailian model capability probe without retaining generated text', async () => {
    ...
    const fetchImpl = vi.fn(async (_url: string, init: RequestInit) => {
      ...
      return new Response(JSON.stringify({ id: 'probe-request-001', object: 'chat.completion', created: 1787788800, model: 'qwen-plus', choices: [{ index: 0, message: { role: 'assistant', content: 'OK' }, finish_reason: 'stop' }], usage: { prompt_tokens: 12, completion_tokens: 1, total_tokens: 13 } }), { status: 200 })
    })
  ...
  it('signs and validates exact internal_hmac_v1 checkout lineage and rejects response identity drift', async () => {
    ...
    const adapter = createInternalHmacV1CheckoutAdapter({ endpointOrigin: 'https://payments.acme-payments.com', providerKey: 'internal_hmac_v1', credentialReference: 'vault:payments', resolveCredential: async () => ({ ok: true, value: credential }), fetchImpl: fetchImpl as typeof fetch, clock: () => now, } as any)
