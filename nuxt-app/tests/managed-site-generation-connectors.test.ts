...
describe('managed-site provider registry and generation admission', () => {
  it('keeps configured separate from verified and never projects credential values', async () => {
    ...
    await verifyManagedSiteProviderConfiguration(1, 'website_generator', async input => ({ capability: 'website_generator', providerKey: input.configuration.providerKey, configurationFingerprint: live.state.configurations[0]!.configurationFingerprint, providerAccountId: 'account-prod-001', exactResponseIdentity: 'verification-response-001', observedAt: '2026-08-27T00:00:00.000Z' }), live.repository, resolver, () => new Date('2026-08-27T00:01:00.000Z'))
