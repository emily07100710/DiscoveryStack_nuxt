...
const ownerActor = (ownerUserId: number) => ({ ownerUserId, actorUserId: ownerUserId, authority: 'owner_session' as const, role: 'owner' as const, principal: `owner-${ownerUserId}@example.test` })

async function makeProject(ownerUserId = 1) {
  const test = createManagedSiteMemoryRepository()
  const result = await createManagedSiteProject(ownerUserId, ownerActor(ownerUserId), { canonicalClientIdentity: `client-${ownerUserId}`, canonicalWebsiteIdentity: `https://client-${ownerUserId}.example.test`, siteType: 'brand_blog', idempotencyKey: `project-key-${ownerUserId}` }, test.repository)
...
describe('managed site project vault', () => {
  it('creates an owner-scoped project and replays idempotently without a duplicate membership', async () => {
    const test = createManagedSiteMemoryRepository()
    const first = await createManagedSiteProject(1, ownerActor(1), { canonicalClientIdentity: 'acme', canonicalWebsiteIdentity: 'https://acme.example.test', siteType: 'one_page', idempotencyKey: 'project-acme-1' }, test.repository)
    const replay = await createManagedSiteProject(1, ownerActor(1), { canonicalClientIdentity: 'acme', canonicalWebsiteIdentity: 'https://acme.example.test', siteType: 'one_page', idempotencyKey: 'project-acme-1' }, test.repository)
  ...
  it('isolates owners and rejects access to another owner project', async () => {
    const test = createManagedSiteMemoryRepository()
    const first = await createManagedSiteProject(1, ownerActor(1), { canonicalClientIdentity: 'client-one', canonicalWebsiteIdentity: 'https://client-one.example.test', siteType: 'brand_blog', idempotencyKey: 'project-one-1' }, test.repository)
    const second = await createManagedSiteProject(2, ownerActor(2), { canonicalClientIdentity: 'client-two', canonicalWebsiteIdentity: 'https://client-two.example.test', siteType: 'brand_blog', idempotencyKey: 'project-two-1' }, test.repository)
  ...
  it('creates immutable version lineage and never includes source code in customer export', async () => {
    ...
    const invitation = await inviteManagedSiteMember(1, test.project.id, ownerActor(1), { email: 'editor@example.test', role: 'editor', idempotencyKey: 'invite-editor-1' }, test.repository)
  ...
  it('uses fixed roles, prevents owner downgrade, and records changes and revocation', async () => {
    const test = await makeProject(1)
    const invitation = await inviteManagedSiteMember(1, test.project.id, ownerActor(1), { email: 'reviewer@example.test', role: 'reviewer', idempotencyKey: 'invite-reviewer-1' }, test.repository)
    const memberId = test.state.memberships.find(row => row.principalEmail === 'reviewer@example.test')!.id
  ...
  it('fails closed for expired invitations and invitation replay', async () => {
    const test = await makeProject(1)
    const invitation = await inviteManagedSiteMember(1, test.project.id, ownerActor(1), { email: 'admin@example.test', role: 'administrator', idempotencyKey: 'invite-admin-1' }, test.repository)
    const accepted = await acceptManagedSiteInvitation(invitation.invitationToken!, test.repository)
    ...
    const expired = await inviteManagedSiteMember(1, test.project.id, ownerActor(1), { email: 'expired@example.test', role: 'analyst', idempotencyKey: 'invite-expired-1' }, test.repository)
