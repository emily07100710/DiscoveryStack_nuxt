...
describe('content operations execution orchestrator', () => {
  ...
  it('supports owner target pause, revoke, and reactivation without exposing credential material', async () => {
    ...
    expect(fixture.targets.find(row => row.id === target.id)?.activeSlot).toBeNull()
    await expect(updateOwnerPublicationTarget(1, target.id, { status: 'active' }, fixture.repository)).rejects.toThrow(/terminal/)
