...
async function createPaidPreview(ownerUserId: number, draftIdentity: string, existingSiteUrl?: string, diagnosisId?: number, resolver?: ExistingSiteDiagnosisResolver) {
  ...
  const quote = await createManagedSiteQuote({ previewId: preview.preview.id, previewAccessToken: preview.accessToken!, planKey: 'basic', cadenceDays: 7, domainOption: existingSiteUrl ? 'existing' : 'new', idempotencyKey: `${draftIdentity}-quote` }, ordering.repository, acceptanceClock)
