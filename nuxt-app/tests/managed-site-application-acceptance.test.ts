...
async function createPaidPreview(ownerUserId: number, draftIdentity: string, existingSiteUrl?: string, diagnosisId?: number, resolver?: ExistingSiteDiagnosisResolver) {
  ...
  const acceptanceClock = () => new Date('2026-08-27T00:00:00.000Z')
  ...
  const preview = await createManagedSitePreview(ownerUserId, brief, ordering.repository, acceptanceClock, resolver)
  const quote = await createManagedSiteQuote({ previewId: preview.preview.id, previewAccessToken: preview.accessToken!, planKey: 'site_geo', cadenceDays: 7, domainOption: existingSiteUrl ? 'existing' : 'new', ...(existingSiteUrl ? {} : { domainTld: 'com' as const }), idempotencyKey: `${draftIdentity}-quote` }, ordering.repository, acceptanceClock)
  ...
  const order = await createManagedSiteDraftOrder({ previewId: preview.preview.id, previewAccessToken: preview.accessToken!, quoteId: quote.quote.quoteId, leadIntentId: lead.leadIntent.id, idempotencyKey: `${draftIdentity}-order` }, ordering.repository, acceptanceClock)
