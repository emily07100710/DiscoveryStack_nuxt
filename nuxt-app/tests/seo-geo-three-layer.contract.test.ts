...
const exportRoute = readFileSync(resolve(process.cwd(), 'server/api/seo-geo/production-plans/[id]/export/[deliverableId].post.ts'), 'utf8')
const revisionRoute = readFileSync(resolve(process.cwd(), 'server/api/seo-geo/production-plans/[id]/revisions/[deliverableId].post.ts'), 'utf8')
...
describe('SEO/GEO three-layer V1 contract', () => {
  ...
  it('keeps the primary workbench as a three-step flow and leaves technical ID forms advanced-only', () => {
    ...
    expect(pageSource).toContain('guidedExport')
  ...
  it('keeps standalone briefs free of client canonical linkage and exposes approved-only export', () => {
    ...
    expect(revisionRoute).toContain('requireOwner(event)')
  })
})
