...
describe('System Factory migration and route contracts', () => {
  it('generates all durable tables without destructive SQL or oversized identifiers', () => { for (const table of expectedTables) expect(migration).toContain(`CREATE TABLE \`${table}\``); expect(migration).not.toMatch(/^\s*(?:DROP|TRUNCATE|DELETE\s+FROM)\b/imu); expect(migration).toContain('stb_payment_event_fk'); for (const name of migration.matchAll(/(?:CONSTRAINT|INDEX) `([^`]+)`/gu)) expect(Buffer.byteLength(name[1] || '', 'utf8')).toBeLessThanOrEqual(64) })
  it('audits every migration identifier and freezes legacy implicit schema foreign keys', () => {
    const output = execFileSync(process.execPath, ['scripts/mysql-identifier-policy.mjs'], { cwd: process.cwd(), encoding: 'utf8' })
    const journal = JSON.parse(readFileSync(join(process.cwd(), 'server/database/migrations/meta/_journal.json'), 'utf8')) as { entries: unknown[] }
    const map = JSON.parse(readFileSync(join(process.cwd(), 'server/database/migrations/mysql-identifier-map.json'), 'utf8')) as { corrections: unknown[]; schemaPolicy: { legacyImplicitForeignKeyCount: number } }
    expect(JSON.parse(output)).toMatchObject({ ok: true, migrations: journal.entries.length, corrections: map.corrections.length, legacyImplicitForeignKeys: map.schemaPolicy.legacyImplicitForeignKeyCount })
  })
