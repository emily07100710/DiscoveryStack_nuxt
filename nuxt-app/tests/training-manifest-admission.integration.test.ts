...
function humanAnnotationMembersWithLocaleDuplicate() {
  return Array.from({ length: 101 }, (_, index) => ({
    artifactId: index + 1,
    sourceId: 1,
    sourceUrl: index === 100
      ? 'https://developers.google.com/search/docs/page-1?hl=zh-tw'
      : `https://developers.google.com/search/docs/page-${index + 1}?hl=en`,
    sourceSpanHash: `annotation-span-${index + 1}`,
    fieldData: {},
...
describe('Training manifest admission for bounded crawl artifacts', () => {
  ...
  it('rejects a manifest that repeats one Google Search Central document through another hl language variant', async () => {
    selectResults = [
      [{ id: 9, intendedUse: 'training', status: 'ready_for_review', labelTaxonomyVersion: SEO_GEO_LABEL_TAXONOMY_VERSION }],
      humanAnnotationMembersWithLocaleDuplicate(),
    ]

    await expect(approveOwnerPublicDatasetBuild({ ownerUserId: 1, datasetBuildId: 9, reviewNote: 'owner review' }))
      .rejects.toMatchObject({ statusCode: 422, statusMessage: 'The manifest contains duplicate canonical source documents or source-span fingerprints and cannot be approved.' })
  })
})
