...
export class ContentOperationsFixture {
  ...
  constructor() {
    this.repository = {
      ...
      listEntityStrategyProfiles: async (owner, clientId, websiteId) => this.entityStrategyProfiles.filter(row => row.ownerUserId === owner && (clientId === undefined || row.clientId === clientId) && (websiteId === undefined || row.websiteId === websiteId)).sort((a, b) => b.version - a.version),
      insertEntityStrategyProfile: async input => { const existing = this.entityStrategyProfiles.find(row => row.ownerUserId === input.ownerUserId && row.profileFingerprint === input.profileFingerprint); if (existing) return existing; if (input.activeScopeKey && this.entityStrategyProfiles.some(row => row.ownerUserId === input.ownerUserId && row.activeScopeKey === input.activeScopeKey)) throw Object.assign(new Error('active entity profile scope collision'), { code: 'ER_DUP_ENTRY' }); const row = { ...input, id: ++this.nextId, createdAt: now(), updatedAt: now() } as ContentOperationEntityStrategyProfileRow; this.entityStrategyProfiles.push(row); return row },
      revokeEntityStrategyProfile: async (owner, profileId, revokedAt) => { const row = this.entityStrategyProfiles.find(item => item.ownerUserId === owner && item.profileId === profileId && item.status === 'active'); if (!row) return null; Object.assign(row, { status: 'revoked', revokedAt, activeScopeKey: null, updatedAt: now() }); return row },
      ...
      listQueryOwnership: async (owner, clientId, websiteId) => this.queryOwnership.filter(row => row.ownerUserId === owner && (clientId === undefined || row.clientId === clientId) && (websiteId === undefined || row.websiteId === websiteId)).sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime()),
      insertQueryOwnership: async input => { const existing = this.queryOwnership.find(row => row.ownerUserId === input.ownerUserId && row.fingerprint === input.fingerprint); if (existing) return existing; if (input.activeScopeKey && this.queryOwnership.some(row => row.ownerUserId === input.ownerUserId && row.activeScopeKey === input.activeScopeKey)) throw Object.assign(new Error('active canonical query owner collision'), { statusCode: 409 }); const row = { ...input, id: ++this.nextId, createdAt: now() } as ContentOperationQueryOwnershipRow; this.queryOwnership.push(row); return row },
      revokeQueryOwnership: async (owner, fingerprint, revokedAt) => { const row = this.queryOwnership.find(item => item.ownerUserId === owner && item.fingerprint === fingerprint && item.status === 'active'); if (!row) return null; Object.assign(row, { status: 'revoked', revokedAt, activeScopeKey: null }); return row },
      ...
      claimRepairAttempt: async (owner, repairFingerprint, leaseOwner, at, leaseExpiresAt) => { const row = this.repairAttempts.find(item => item.ownerUserId === owner && item.repairFingerprint === repairFingerprint && item.status === 'planned' && (!item.leaseOwner || !item.leaseExpiresAt || item.leaseExpiresAt < at)); if (!row) return null; Object.assign(row, { leaseOwner, leaseExpiresAt }); return row },
      updateRepairAttempt: async (owner, repairFingerprint, patch) => { const row = this.repairAttempts.find(item => item.ownerUserId === owner && item.repairFingerprint === repairFingerprint); if (!row) throw new Error('repair attempt not found'); if (patch.repairedDraftId !== undefined) row.repairedDraftId = patch.repairedDraftId; if (patch.repairedContentHash !== undefined) row.repairedContentHash = patch.repairedContentHash; if (patch.status !== undefined) Object.assign(row, { status: patch.status, leaseOwner: null, leaseExpiresAt: null }); return row },
      ...
      findMachineAuthorizationForTarget: async (owner, entryId, targetRowId) => [...this.machineAuthorizations].reverse().find(row => row.ownerUserId === owner && row.entryId === entryId && row.publicationTargetId === targetRowId) || null,
      ...
      transitionMachineAuthorization: async (owner, fingerprint, fromStatus, toStatus, at) => { const row = this.machineAuthorizations.find(item => item.ownerUserId === owner && item.authorizationFingerprint === fingerprint && item.status === fromStatus); if (!row) return null; Object.assign(row, { status: toStatus, ...(toStatus === 'executing' ? { claimedAt: at || now() } : {}), ...(toStatus === 'revoked' ? { revokedAt: at || now() } : {}) }); return row },
      reserveAutopilotBudget: async input => { const existing = this.budgetReservations.find(row => row.ownerUserId === input.ownerUserId && row.policyId === input.policyId && row.kind === input.kind && row.idempotencyKey === input.idempotencyKey); const policy = this.autopilotPolicies.find(row => row.ownerUserId === input.ownerUserId && row.policyId === input.policyId && row.publicationTargetId === input.publicationTargetId); if (!policy) return { reserved: false, replayed: false, reservation: null, used: 0, limit: 0 }; const usedKey = input.kind === 'generation' ? 'generationBudgetUsed' : 'publicationBudgetUsed'; const limit = input.kind === 'generation' ? policy.generationBudget : policy.publicationBudget; const used = Number(policy[usedKey] || 0); if (existing) { if (existing.inputFingerprint !== input.inputFingerprint || existing.entryId !== input.entryId || existing.publicationTargetId !== input.publicationTargetId || existing.units !== input.units) throw Object.assign(new Error('budget collision'), { statusCode: 409 }); return { reserved: true, replayed: true, reservation: existing, used, limit } } if (policy.status !== 'enabled' || used + input.units > limit) return { reserved: false, replayed: false, reservation: null, used, limit }; policy[usedKey] = used + input.units; const row = { ...input, id: ++this.nextId, createdAt: now() } as ContentOperationBudgetReservationRow; this.budgetReservations.push(row); return { reserved: true, replayed: false, reservation: row, used: used + input.units, limit } },
      listAutopilotBudgetReservations: async (owner, policyId) => this.budgetReservations.filter(row => row.ownerUserId === owner && (policyId === undefined || row.policyId === policyId)),
