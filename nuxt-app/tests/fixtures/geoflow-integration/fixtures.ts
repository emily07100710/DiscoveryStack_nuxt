...
export const validRequestDraft: GeoFlowRequestDraft = {
  ...
  evidenceChunks: [{ sourceId: 'source-1', artifactId: 'artifact-1', chunkId: 'chunk-1', chunkHash: CHUNK_HASH, reviewedText: 'Approved evidence text.', locator: 'https://evidence.routing.discoverystack.dev/research/chunk-1' }], createdAt: '2026-08-25T04:00:00+08:00',
}
...
export function makeProgressResponse(request: GeoFlowRequest = validRequest, status: 'queued' | 'running' = 'running', attempt = 1): ProgressResponse { return { protocolVersion: request.protocolVersion, requestId: request.requestId, idempotencyKey: request.idempotencyKey, requestFingerprint: request.requestFingerprint, ownerUserId: request.ownerUserId, clientId: request.clientId, jobId: request.jobId, externalProjectKey: 'project-1', externalTaskKey: 'task-1', externalJobKey: 'job-1', externalArticleKey: `article-${request.calendarEntryId}-${request.deliverableId}`, attempt, status, observedAt: '2026-08-25T04:01:00.000Z', limitations: [], retry: null } }
export function makeRetryResponse(request: GeoFlowRequest = validRequest, attempt = 1): RetryWaitResponse { return { ...makeProgressResponse(request, 'running', attempt), status: 'retry_wait', retry: { attempt, retryAt: '2026-08-25T04:05:00.000Z' } } }
export function makeFailureResponse(request: GeoFlowRequest = validRequest, status: 'blocked' | 'failed' = 'failed', attempt = 1, retryable = false): FailureResponse { return { protocolVersion: request.protocolVersion, requestId: request.requestId, idempotencyKey: request.idempotencyKey, requestFingerprint: request.requestFingerprint, ownerUserId: request.ownerUserId, clientId: request.clientId, jobId: request.jobId, externalProjectKey: 'project-1', externalTaskKey: 'task-1', externalJobKey: 'job-1', externalArticleKey: `article-${request.calendarEntryId}-${request.deliverableId}`, attempt, status, observedAt: '2026-08-25T04:01:00.000Z', failure: { code: 'INVALID_INPUT', retryable }, limitations: ['Candidate generation did not complete.'] } }
export function makeDraftResponse(request: GeoFlowRequest = validRequest, overrides: Partial<DraftResultResponse> = {}): DraftResultResponse { const articleKey = `article-${request.calendarEntryId}-${request.deliverableId}`; return { protocolVersion: request.protocolVersion, requestId: request.requestId, idempotencyKey: request.idempotencyKey, requestFingerprint: request.requestFingerprint, ownerUserId: request.ownerUserId, clientId: request.clientId, jobId: request.jobId, externalProjectKey: 'project-1', externalTaskKey: 'task-1', externalJobKey: 'job-1', externalArticleKey: articleKey, attempt: 1, status: 'review_required', draftIdentity: { externalArticleKey: articleKey, briefFingerprint: request.briefFingerprint }, contentArtifact: { ...validContentArtifact, contentType: request.contentType, language: request.language }, evidenceSnapshotHash: request.evidenceSnapshotHash, citationBindings: [{ sourceId: 'source-1', artifactId: 'artifact-1', chunkId: 'chunk-1', chunkHash: CHUNK_HASH }], appliedRuleIds: [...request.selectedRuleIds], providerProvenance: { provider: 'deterministic_scaffold', model: 'none', mode: 'deterministic_scaffold', fallbackReason: null }, limitations: ['No external provider generation was executed.'], completedAt: '2026-08-25T04:01:00.000Z', ...overrides } }
