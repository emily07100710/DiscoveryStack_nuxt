...
export function createManagedSiteMemoryRepository() {
  ...
  const make = (): ManagedSiteRepository => ({
    ...
    async insertVersion(input) {
      if (state.versions.some(row => row.projectId === input.projectId && (row.version === input.version || row.versionFingerprint === input.versionFingerprint))) throw Object.assign(new Error('duplicate version'), { code: 'ER_DUP_ENTRY', errno: 1062 })
      return create(state.versions, input as Omit<ManagedSiteVersion, 'id'>)
    },
