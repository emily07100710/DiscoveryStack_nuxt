...
type State = {
  ...
  leads: Array<{ id: number; requestFingerprint: string }>
  nextId: number
}
...
export function createOrderingMemoryRepository() {
  ...
  const make = (): PreviewRepository => ({
    async transaction(work) { const saved = snapshot(); try { return await work(make()) } catch (error) { restore(saved); throw error } },
    ...
    async insertLead(input) { const row = { id: state.nextId++, requestFingerprint: input.requestFingerprint }; state.leads.push(row); return { id: row.id } },
    async findLeadIntentByIdempotency(key) { return state.leadIntents.find(row => row.idempotencyKey === key) || null },
    ...
    async findDraftOrderByIdempotency(key) { return state.orders.find(row => row.idempotencyKey === key) || null },
    ...
    async findPaymentEvent(eventId) { return state.paymentEvents.find(row => row.eventId === eventId) || null },
