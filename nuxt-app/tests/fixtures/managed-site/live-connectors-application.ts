...
// Keep deterministic fixtures ahead of the wall clock so route-level expiry checks
// remain meaningful without silently weakening the production TTL boundary.
export const managedSiteFixedNow = new Date('2030-01-01T00:00:00.000Z')
...
export async function createAuthoritativeManagedSiteReleaseFixture(options: { ownerUserId?: number; siteType?: 'one_page' | 'brand_blog' | 'simple_commerce'; selectedModules?: any[]; canonicalDomain?: string; buildPreview?: boolean; createCheckout?: boolean } = {}) {
  ...
  const quote = await createManagedSiteQuote({ previewId: preview.preview.id, previewAccessToken: preview.accessToken!, planKey: 'site_geo', cadenceDays: 7, domainOption: 'new', domainTld: 'com', idempotencyKey: 'fixture-quote-001' }, ordering.repository, () => managedSiteFixedNow)
