...
export function createLiveConnectorMemoryRepository() {
  const state: State = { configurations: [], candidates: [], releases: [], attempts: [], receipts: [], nextId: 1 }
  ...
  const make = (): ManagedSiteLiveConnectorRepository => ({
    ...
    async updateProviderConfiguration(id, patch) { const row = state.configurations.find(item => item.id === id); if (!row) return null; Object.assign(row, patch, { updatedAt: new Date() }); return row },
    ...
    async updateRelease(ownerUserId, releaseId, patch) { const row = state.releases.find(item => item.ownerUserId === ownerUserId && item.id === releaseId); if (!row) return null; Object.assign(row, patch, { updatedAt: new Date() }); return row },
