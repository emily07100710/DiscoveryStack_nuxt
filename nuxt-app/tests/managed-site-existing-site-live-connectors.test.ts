...
import { activateManagedSiteGeoOperations, createExistingSiteOwnershipChallenge, createExistingSiteRelease, createMockExistingSiteOwnershipAdapter, verifyExistingSiteOwnership } from '../server/managed-sites/live-connectors/deployment-orchestrator'
...
describe('managed-site existing-site path and tenant isolation', () => {
  it('requires verified ownership before canonical GEO activation and blocks cross-owner/project domain attacks', async () => {
    ...
    const challenge = await createExistingSiteOwnershipChallenge(1, { releaseId: release.release.id, idempotencyKey: 'ownership-challenge-001' }, live.repository, () => new Date('2026-08-27T00:00:00.000Z'))
    const verified = await verifyExistingSiteOwnership(1, { releaseId: release.release.id, challengeReceiptFingerprint: challenge.receipt.receiptFingerprint, executionMode: 'mocked', idempotencyKey: 'ownership-verify-001' }, createMockExistingSiteOwnershipAdapter(), { repository: live.repository, clock: () => new Date('2026-08-27T00:00:00.000Z') })
    expect(verified.release?.status).toBe('live_verified')
    await expect(verifyExistingSiteOwnership(2, { releaseId: release.release.id, challengeReceiptFingerprint: challenge.receipt.receiptFingerprint, executionMode: 'mocked', idempotencyKey: 'cross-owner-verify' }, createMockExistingSiteOwnershipAdapter(), { repository: live.repository })).rejects.toMatchObject({ statusCode: 409 })
    ...
    await expect(createExistingSiteOwnershipChallenge(1, { releaseId: conflicting.release.id, idempotencyKey: 'ownership-conflict' }, live.repository)).rejects.toMatchObject({ statusCode: 409 })
