...
describe('managed-site explicit active domain claim release', () => {
  it('retains history, frees only the nullable active key, and permits a later atomic claim', async () => {
    ...
    const claim = await line.live.repository.insertDomainClaim({ canonicalDomain: release!.canonicalDomain, ownerUserId: line.ownerUserId, projectId: release!.projectId, releaseId: release!.id, claimKind: 'generated', status: 'pending', authorityReceiptFingerprint: null, requestFingerprint, idempotencyKey: 'claim-before-release', projectionFingerprint } as any)
    ...
    const later = await line.live.repository.insertDomainClaim({ canonicalDomain: release!.canonicalDomain, ownerUserId: 2, projectId: 200, releaseId: 201, claimKind: 'existing', status: 'pending', authorityReceiptFingerprint: null, requestFingerprint: stableFingerprint({ later: true }), idempotencyKey: 'later-cross-owner-claim', projectionFingerprint: stableFingerprint({ later: 'pending' }) } as any)
  ...
  it('fails closed while a domain is live or governed deployment/rollback is active', async () => {
    ...
    const claim = await line.live.repository.insertDomainClaim({ canonicalDomain: release!.canonicalDomain, ownerUserId: line.ownerUserId, projectId: release!.projectId, releaseId: release!.id, claimKind: 'generated', status: 'verified', authorityReceiptFingerprint: 'a'.repeat(64), requestFingerprint, idempotencyKey: 'live-domain-claim', projectionFingerprint } as any)
