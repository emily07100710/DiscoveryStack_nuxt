...
describe('managed-site payment conversion orchestration', () => {
  it('resolves a public order owner from the existing account and binds all lineage before conversion', async () => {
    ...
    expect(result.authority).toMatchObject({ ownerUserId: 1, source: 'existing_account_email' })
  ...
  it('replays payment and conversion independently after a process interruption between the two stages', async () => {
    ...
    const firstPayment = await import('../server/managed-sites/ordering-service').then(module => module.recordVerifiedPaymentEvent(input, verifier, ordering.repository))
    expect(firstPayment.replayed).toBe(false)
    expect(ordering.state.paymentEvents).toHaveLength(1)

    const recovered = await processManagedSitePaymentAndConversion(input, verifier, { ordering: ordering.repository, managed: managed.repository })
    ...
    const replay = await processManagedSitePaymentAndConversion(input, verifier, { ordering: ordering.repository, managed: managed.repository })
