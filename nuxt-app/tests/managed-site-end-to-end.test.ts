...
it('runs the managed platform path end to end with server-owned payment conversion and injected provider boundaries', async () => {
  ...
  const assistant = await runManagedSiteAssistant(1, { projectId: conversion.project.id, question: '這個網站何時可以上線？' }, undefined, managed.repository)
