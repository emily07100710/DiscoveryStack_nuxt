...
it('runs the managed platform path end to end with server-owned payment conversion and injected provider boundaries', async () => {
  ...
  const quote = await createManagedSiteQuote({ previewId: preview.preview.id, previewAccessToken: preview.accessToken!, planKey: 'site_geo_autopost', cadenceDays: 7, domainOption: 'new', domainTld: 'com', idempotencyKey: 'e2e-quote-1' }, ordering.repository)
