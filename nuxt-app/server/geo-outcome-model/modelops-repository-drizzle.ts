...
import { geoOutcomeModelopsCycles, geoOutcomeModelopsEvents, geoOutcomeModelopsPolicies, geoOutcomeModelopsRollbackDecisions, geoOutcomeModelopsShadowEvaluations } from '../database/schema'
import { fingerprint } from './canonical'
import type { ModelFamily } from './constants'
import type { ModelOpsCycle, ModelOpsCycleClaimResult, ModelOpsEvent, ModelOpsPolicy, ModelOpsRepositoryPort, ModelOpsRollbackDecision, ModelOpsShadowEvaluation } from './modelops-types'
...
export class DrizzleModelOpsRepository implements ModelOpsRepositoryPort {
  ...
  async updatePolicy(ownerUserId: number, policyId: string, patch: Partial<ModelOpsPolicy>) {
    ...
    for (const key of ['status', 'cadence', 'minimumNewVerifiedCandidates', 'minimumNewQueryGroups', 'minimumNewWebsites', 'minimumObservationSpanDays', 'maximumTrainingRunsPerCycle', 'cooldownHours', 'shadowEvaluationEnabled', 'authorizedByOwnerUserId', 'configurationFingerprint', 'revokedAt'] as const) if (patch[key] !== undefined) values[key] = patch[key]
