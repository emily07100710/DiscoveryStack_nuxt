...
export interface NormalizeOptions { mode?: 'intake' | 'trusted_test'; consentStatus?: 'approved' | 'revoked' | 'unknown'; piiStatus?: 'clean' | 'contains_pii' | 'unknown'; verificationAuthority?: OutcomeObservation['verificationAuthority']; reviewFingerprint?: string | null }
...
export function observationFingerprintPayload(observation: Omit<OutcomeObservation, 'observationFingerprint'>): unknown {
  const { verificationStatus, consentStatus, piiStatus, verificationAuthority, reviewFingerprint, ...immutable } = observation
  void verificationStatus; void consentStatus; void piiStatus; void verificationAuthority; void reviewFingerprint
  return immutable
}
...
export function isHardNegativeCandidate(candidate: OutcomeObservation, positive: OutcomeObservation): boolean {
  ...
  if (!canBePrimaryCitationTruth(candidate) || candidate.citationStatus !== 'not_cited' || candidate.observationFingerprint === positive.observationFingerprint) return false
  return true
}
