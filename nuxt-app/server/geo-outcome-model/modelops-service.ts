...
import type { DatasetManifest, DatasetMember, EvaluationBundle, GeoOutcomeRepositoryPort, ModelArtifact, TrainingRun } from './types'
...
function policyUsable(policy: ModelOpsPolicy, trigger: ModelOpsCycle['trigger']): boolean { if (policy.status === 'revoked') return false; if (policy.expiresAt && new Date(policy.expiresAt).getTime() <= Date.now()) return false; return trigger === 'owner_manual' || trigger === 'dry_run' || policy.status === 'enabled' }
...
export async function dryRunModelOpsCycle(ownerUserId: number, trigger: 'owner_manual' | 'scheduled' | 'dry_run', outcomeRepository: GeoOutcomeRepositoryPort, modelOpsRepository: ModelOpsRepositoryPort, at = new Date()): Promise<ModelOpsDryRunPlan> {
  ...
  const thresholdReady = Boolean(policy && policyUsable(policy, trigger) && !cooldownActive && counts.candidates >= policy.minimumNewVerifiedCandidates && counts.queryGroups >= policy.minimumNewQueryGroups && counts.websites >= policy.minimumNewWebsites && (counts.observationSpanDays === null || counts.observationSpanDays < policy.minimumObservationSpanDays ? false : true))
...
export async function createModelOpsCycle(ownerUserId: number, trigger: ModelOpsCycle['trigger'], idempotencyKey: string, outcomeRepository: GeoOutcomeRepositoryPort, modelOpsRepository: ModelOpsRepositoryPort, at = new Date()): Promise<ModelOpsCycle> {
  assertOwner(ownerUserId); const policies = await modelOpsRepository.listPolicies(ownerUserId); const policy = policies.sort((a, b) => a.updatedAt.localeCompare(b.updatedAt)).at(-1); if (!policy) throw new Error('ModelOps policy is not configured.')
  if (!policyUsable(policy, trigger)) throw new Error(policy.status === 'enabled' ? 'ModelOps policy is expired.' : 'ModelOps policy is paused or revoked.')
...
export async function evaluateModelOpsShadow(ownerUserId: number, artifactId: string, outcomeRepository: GeoOutcomeRepositoryPort, modelOpsRepository: ModelOpsRepositoryPort, at = new Date()): Promise<ModelOpsShadowEvaluation> {
  assertOwner(ownerUserId); const artifact = await outcomeRepository.getArtifact(ownerUserId, artifactId); if (!artifact) throw new Error('Model artifact not found.'); if (!verifyArtifactHash(artifact)) throw new Error('Artifact hash mismatch.'); if (artifact.status !== 'approved_for_shadow') throw new Error('Only owner-approved shadow artifacts may be evaluated.')
  ...
  const members = await outcomeRepository.getDatasetMembers(ownerUserId, dataset.manifestId); const trainingSet = new Set(dataset.sourceObservationFingerprints); const observations = primaryObservations(await outcomeRepository.listObservations(ownerUserId)).filter(item => !trainingSet.has(item.observationFingerprint) && new Date(item.runTimestamp).getTime() > new Date(dataset.observationEnd || 0).getTime()); const built = buildCitationSelectionDataset(observations, ownerUserId); const shadowMembers = built.members; const split = built.split || { train: [], validation: [], test: [], siteHoldout: [], queryHoldout: [], temporalHoldout: [] }; const parameters: TrainedParameters = { coefficients: artifact.coefficients, intercept: artifact.intercept, normalizationStatistics: artifact.normalizationStatistics, trainingRowCount: artifact.trainingRowCount, featureKeys: artifact.coefficients.map((_, index) => `feature_${index}`), trainingConfiguration: artifact.trainingConfiguration }; const metrics = evaluateModel(parameters, shadowMembers, split, dataset.taskType); const counts = summary(observations); const predictionCounts = predictedClassCounts(artifact, shadowMembers); const prior = (await modelOpsRepository.listShadowEvaluations(ownerUserId, artifactId)).at(-1) || null; const severe = shadowMembers.length > 0 && (predictionCounts.positive === 0 || predictionCounts.negative === 0 || Boolean(prior?.binaryMetrics && typeof prior.binaryMetrics.f1 === 'number' && typeof metrics.test.f1 === 'number' && metrics.test.f1 < (prior.binaryMetrics.f1 as number) - 0.2)); const readiness = getDatasetReadiness(counts); const status: ModelOpsShadowEvaluation['status'] = severe ? 'needs_owner_attention' : !shadowMembers.length || !readiness.ready ? 'insufficient_data' : 'completed'; const evaluationWindowStart = observations.length ? observations.map(item => item.runTimestamp).sort()[0]! : (dataset.observationEnd || at.toISOString()); const evaluationWindowEnd = observations.length ? observations.map(item => item.runTimestamp).sort().at(-1)! : at.toISOString(); const reasonCodes = [...(!shadowMembers.length ? ['no_new_shadow_candidates'] : []), ...(!readiness.ready ? ['insufficient_data'] : []), ...(predictionCounts.positive === 0 || predictionCounts.negative === 0 ? ['zero_prediction_class'] : []), ...(severe && predictionCounts.positive > 0 && predictionCounts.negative > 0 ? ['metrics_regression_or_drift'] : [])]; const payload = { ownerUserId, artifactId, artifactHash: artifact.artifactHash, evaluationWindowStart, evaluationWindowEnd, observationFingerprints: shadowMembers.map(item => item.observationFingerprint).sort(), candidateCount: shadowMembers.length, positiveCount: shadowMembers.filter(item => item.label === 1).length, negativeCount: shadowMembers.filter(item => item.label === 0).length, queryGroupCount: counts.queryGroups, websiteCount: counts.websites, engineCounts: distribution(observations), binaryMetrics: metricObject(metrics), rankingMetrics: { validation: metrics.rankingValidation, test: metrics.rankingTest, temporalHoldout: metrics.rankingTemporalHoldout }, calibrationDiagnostics: { testBrierScore: metrics.test.brierScore, testExpectedCalibrationError: metrics.test.expectedCalibrationError, zeroDenominatorValuesAreNull: true }, driftDiagnostics: { currentEngineCounts: distribution(observations), baselineEngineCounts: dataset.engineCounts, predictionClassCounts: predictionCounts, trainingObservationOverlap: shadowMembers.some(item => trainingSet.has(item.observationFingerprint)) }, status, reasonCodes: [...new Set(reasonCodes)] }; const evaluationFingerprint = fingerprint(payload); if (severe) await outcomeRepository.markArtifactShadowFailed(ownerUserId, artifactId); const evaluation: ModelOpsShadowEvaluation = { ...payload, evaluationId: `geo-modelops-shadow-${evaluationFingerprint.slice(0, 24)}`, evaluationFingerprint, createdAt: at.toISOString() }; const saved = await modelOpsRepository.saveShadowEvaluation(ownerUserId, evaluation); await append(modelOpsRepository, ownerUserId, `shadow-${artifactId}`, 'shadow_evaluation_completed', { evaluationId: saved.evaluationId, status: saved.status, reasonCodes: saved.reasonCodes }); return saved
}
...
export async function rollbackModelOpsArtifact(ownerUserId: number, artifactId: string, rollbackArtifactHash: string, reason: string, outcomeRepository: GeoOutcomeRepositoryPort, modelOpsRepository: ModelOpsRepositoryPort, reviewerUserId = ownerUserId): Promise<{ decision: ModelOpsRollbackDecision, revokedArtifact: Awaited<ReturnType<typeof reviewModel>>['artifact'] }> {
  ...
  const current = await outcomeRepository.getArtifact(ownerUserId, artifactId); if (!current) throw new Error('Model artifact not found.'); if (current.status !== 'approved_for_shadow' && current.status !== 'shadow_failed') throw new Error('Only a shadow artifact requiring rollback may be rolled back.')
