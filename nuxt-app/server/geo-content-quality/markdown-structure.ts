...
function markerIds(value: string): string[] {
  const ids: string[] = []
  for (const match of value.matchAll(CITATION_MARKER)) ids.push(match[1]!)
  return [...new Set(ids)]
}
...
function parseFaqPairs(lines: string[], faqHeadingIndexes: Set<number>): MarkdownFaqPair[] {
  ...
  for (const index of [...faqHeadingIndexes].sort((left, right) => left - right)) {
    ...
    for (let cursor = index + 1; cursor < lines.length; cursor += 1) {
      ...
      const questionMatch = /^(?:(?:Q(?:uestion)?|問(?:題)?)\s*[:：]\s*)?(.{3,}[?？])$/iu.exec(line)
      if (questionMatch) {
        if (question !== null && answerLines.length) pairs.push({ question, answer: answerLines.join(' ').trim() })
        question = questionMatch[1]!.trim()
        answerLines = []
      } else if (question !== null) answerLines.push(line)
    }
    if (question !== null && answerLines.length) pairs.push({ question, answer: answerLines.join(' ').trim() })
  }
  return pairs
...
export function parseMarkdownStructure(value: unknown): MarkdownStructureResult {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (isFence(line)) { inFence = !inFence; flushParagraph(); continue }
    if (inFence) continue
    const heading = isHeading(line)
    if (heading) {
      flushParagraph()
      headings.push({ ...heading, line: index })
      if (titleIncludes(normalizeComparable(heading.title), [/faq/u, /常見問題/u, /常見問答/u])) faqHeadingIndexes.add(index)
      continue
    }
  ...
  for (const heading of headings) { const key = normalizeComparable(heading.title); if (key) duplicateHeadingMap.set(key, (duplicateHeadingMap.get(key) || 0) + 1) }
  ...
  const emptySection = headings.some((heading, index) => {
    ...
    return !paragraphLineIndexes.some(line => line > heading.line && line < nextHeadingLine)
  })
  ...
  const paragraphIdentities: ParagraphIdentity[] = paragraphs.map((paragraph, paragraphIndex) => ({ paragraphIndex, normalizedText: normalizeParagraph(paragraph), paragraphHash: sha256Text(normalizeParagraph(paragraph)), citationMarkerIds: markerIds(paragraph) }))
  ...
  const citationMarkerCount = (prose.match(CITATION_MARKER) || []).length
  const citationMarkerPlacementValid = paragraphs.every(paragraph => markerIds(paragraph).length === 0 || paragraph.length > 10)
  const normalizedHeadingValues = headings.map(heading => normalizeComparable(heading.title))
  ...
  if (!headings.some(heading => heading.level === 1)) reasonCodes.push('INVALID_HEADING_HIERARCHY')
  ...
  return reasonCodes.length ? { status: 'invalid', report, reasonCodes: [...new Set(reasonCodes)] } : { status: 'valid', report, reasonCodes: [] }
}
