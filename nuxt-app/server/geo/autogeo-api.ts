...
${source}${evidence}${diagnosis}${strategy}${brief}
...
export function createAutoGeoApiAdapter(options: AutoGeoApiAdapterOptions = {}): GeoRewriteAdapter {
  ...
  return {
    ...
    async rewrite(document) {
      ...
      try {
        response = await fetchImpl(GEMINI_GENERATE_CONTENT_URL, { method: 'POST', headers: { 'content-type': 'application/json', 'x-goog-api-key': apiKey }, body: JSON.stringify({ contents: [{ role: 'user', parts: [{ text: buildOfficialAutoGeoPrompt(document) }] }] }) })
      ...
      return {
        provider: 'autogeo-api', providerVersion: `autogeo-api@${AUTOGEO_UPSTREAM.revision.slice(0, 12)}+${AUTOGEO_UPSTREAM.model}`, optimizedTitle: document.title, optimizedContent, appliedRuleIds: [],
