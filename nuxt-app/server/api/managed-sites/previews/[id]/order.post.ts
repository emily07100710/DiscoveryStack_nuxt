...
export default defineEventHandler(async (event) => {
  ...
  const body = await strictManagedSiteBody(event, ['previewAccessToken', 'quoteId', 'leadIntentId', 'idempotencyKey'])
  return createManagedSiteDraftOrder({ ...body, previewId } as any, managedSitePublicOrderingRepository())
})
