...
import { createMockManagedSiteDeploymentAdapter, deployManagedSiteProduction } from '../../../../../../managed-sites/live-connectors/deployment-orchestrator'
import { managedSiteLiveDeploymentAdapter } from '../../../../../../managed-sites/live-connectors/runtime-adapters'

export default defineEventHandler(async event => {
  const { ownerUserId, repository, managedRepository, deploymentAdapter } = await managedSiteOwnerContext(event)
  ...
  return deployManagedSiteProduction(ownerUserId, { releaseId, executionMode, idempotencyKey: String(body.idempotencyKey || '') }, adapter, { repository, managedRepository })
})
