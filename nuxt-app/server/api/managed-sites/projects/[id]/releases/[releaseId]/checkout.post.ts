...
export default defineEventHandler(async event => {
  ...
  const adapter = executionMode === 'mocked' ? createMockManagedSiteCheckoutSessionAdapter() : unsupportedManagedSiteVendorAdapter('payment')
  if (!release.draftOrderId) throw createError({ statusCode: 409, statusMessage: 'Release has no exact draft-order lineage.' })
  return createManagedSiteCheckoutSession(ownerUserId, { releaseId, draftOrderId: release.draftOrderId, executionMode, idempotencyKey: String(body.idempotencyKey || '') }, adapter, { connectorRepository: repository })
})
