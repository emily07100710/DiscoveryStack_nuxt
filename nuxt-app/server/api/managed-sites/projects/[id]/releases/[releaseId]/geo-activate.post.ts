...
export default defineEventHandler(async event => {
  const { ownerUserId, repository, managedRepository, geoActivator } = await managedSiteOwnerContext(event)
  ...
  return activateManagedSiteGeoOperations(ownerUserId, { releaseId, timeZone: String(body.timeZone || ''), cadenceDays: Number(body.cadenceDays) as 3 | 7 | 15 | 30, monthlyBudgetUnits: Number(body.monthlyBudgetUnits), idempotencyKey: String(body.idempotencyKey || '') }, { repository, managedRepository, activate: geoActivator as any })
})
