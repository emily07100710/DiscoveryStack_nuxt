...
export default defineEventHandler(async event => {
  const { ownerUserId, repository, managedRepository, domainAdapter } = await managedSiteOwnerContext(event)
  ...
  const adapter = domainAdapter || (executionMode === 'mocked' ? createMockManagedSiteDomainAdapter() : executionMode === 'live' ? await managedSiteLiveDomainAdapter(ownerUserId, repository) : undefined)
  return quoteManagedSiteDomain(ownerUserId, { projectId, releaseId, requestedDomain: String(body.requestedDomain || release.canonicalDomain), executionMode, idempotencyKey: String(body.idempotencyKey || '') }, adapter, { repository, managedRepository })
})
