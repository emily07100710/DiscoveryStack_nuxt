...
export default defineEventHandler(async event => {
  const { ownerUserId, repository, managedRepository } = await managedSiteOwnerContext(event)
  ...
  return createGeneratedManagedSiteRelease(ownerUserId, { projectId, generationCandidateId: Number(body.generationCandidateId), canonicalDomain: String(body.canonicalDomain || ''), targetKey: String(body.targetKey || ''), idempotencyKey: String(body.idempotencyKey || '') }, { repository, managedRepository })
})
