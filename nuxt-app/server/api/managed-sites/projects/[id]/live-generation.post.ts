...
export default defineEventHandler(async event => {
  const { ownerUserId, repository, managedRepository, generationAdapter, artifactVault } = await managedSiteOwnerContext(event)
  ...
  const dependencies = executionMode === 'live' ? { repository, managedRepository, adapter: generationAdapter || await managedSiteLiveGenerationAdapter(ownerUserId, repository), vault: artifactVault || createS3ManagedSiteArtifactVault() } : executionMode === 'mocked' ? { repository, managedRepository, adapter: generationAdapter, vault: artifactVault } : { repository, managedRepository }
