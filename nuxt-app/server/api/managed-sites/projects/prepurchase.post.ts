...
export default defineEventHandler(async event => {
  const { ownerUserId } = await managedSiteOwnerContext(event)
  const body = await strictManagedSiteBody(event, ['previewId', 'quoteId', 'leadIntentId', 'draftOrderId', 'idempotencyKey'])
  const result = await convertClaimedManagedSitePrePurchase(ownerUserId, { previewId: Number(body.previewId), quoteId: Number(body.quoteId), leadIntentId: Number(body.leadIntentId), draftOrderId: Number(body.draftOrderId), idempotencyKey: String(body.idempotencyKey || '') })
