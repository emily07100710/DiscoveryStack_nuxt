...
const inputSchema = z.discriminatedUnion('action', [
  ...
  z.object({
    action: z.literal('run_supervised_training'),
    mode: z.enum(['development', 'production']).default('development'),
    datasetBuildId: z.number().int().positive(),
  }),
])
...
export default defineEventHandler(async (event) => {
  ...
  if (parsed.data.action === 'run_supervised_training') {
    return runSupervisedTraining({
      ownerUserId,
      mode: parsed.data.mode,
      datasetBuildId: parsed.data.datasetBuildId,
    })
  }
