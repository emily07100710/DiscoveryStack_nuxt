...
export default defineEventHandler(async event => {
  ...
  try {
    ...
    const policy = await withMutationIdempotency(ownerUserId, 'geo-outcome-model/modelops/policies/revoke', idempotencyKey, { policyId, body }, async () => changeModelOpsPolicy(ownerUserId, policyId, 'revoke', modelOpsRepository, reason))
    return { status: 'success', policy: projectPolicy(policy), terminal: true, automaticallyReenabled: false }
  } catch (error) {
