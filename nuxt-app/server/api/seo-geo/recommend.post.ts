...
const inputSchema = z.object({
  ...
  idempotencyKey: z.string().trim().min(12).max(256),
})
...
export default defineEventHandler(async event => {
  ...
  const { briefId, idempotencyKey, ...document } = parsed.data
  return runOwnerAutoGeoContentJob({ ownerUserId: await getOwnerDatabaseUserId(owner.openId), briefId, document, idempotencyKey })
})
