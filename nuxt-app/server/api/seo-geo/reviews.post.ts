...
import { createContentReview, getProductionPlanDetail } from '../../seo-geo-core/repository'
...
export default defineEventHandler(async event => {
  ...
  const review = await createContentReview({ ownerUserId, ...parsed.data })
  return review.planId ? { ...review, plan: await getProductionPlanDetail(ownerUserId, review.planId) } : review
})
