...
function makeRepository(database: any): PreviewRepository {
  const repository: PreviewRepository = {
    ...
    async findLeadIntentByIdempotency(key) {
      const [row] = await database.select().from(managedSiteLeadIntents).where(eq(managedSiteLeadIntents.idempotencyKey, key)).limit(1)
      return row || null
    },
    ...
    async findDraftOrderByIdempotency(key) {
      const [row] = await database.select().from(managedSiteDraftOrders).where(eq(managedSiteDraftOrders.idempotencyKey, key)).limit(1)
      return row || null
    },
    ...
    async findPaymentEvent(eventId) {
      const [row] = await database.select().from(managedSitePaymentEvents).where(eq(managedSitePaymentEvents.eventId, eventId)).limit(1)
      return row || null
    },
