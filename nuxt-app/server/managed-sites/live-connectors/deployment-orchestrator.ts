...
async function deploymentProvider(ownerUserId: number, mode: Exclude<ManagedSiteConnectorExecutionMode, 'dry_run'>, repository: ManagedSiteLiveConnectorRepository, resolver: ManagedSiteCredentialResolver) {
  return resolveManagedSiteProviderAuthority(ownerUserId, 'deployment', mode, repository, resolver)
}
...
export async function buildManagedSitePreview(ownerUserId: number, input: { releaseId: number; executionMode: 'mocked' | 'live'; idempotencyKey: string }, adapter: ManagedSiteDeploymentAdapter, dependencies: { repository?: ManagedSiteLiveConnectorRepository; credentialResolver?: ManagedSiteCredentialResolver; clock?: () => Date } = {}) {
  ...
  try {
    const result = await adapter.buildPreview({ projectId: release.projectId, versionId: release.versionId, releaseId: release.id, vaultReference: candidate.vaultReference, contentHash: release.contentHash, canonicalDomain: release.canonicalDomain, providerAuthority, requestFingerprint, timeoutMs: DEPLOYMENT_TIMEOUT_MS })
    validateDeploymentReceipt(result, { providerKey: providerAuthority.providerKey, providerAuthorityFingerprint: providerAuthority.authorityFingerprint, projectId: release.projectId, versionId: release.versionId, contentHash: release.contentHash, canonicalDomain: release.canonicalDomain, status: 'preview_ready' })
...
export async function createExistingSiteOwnershipChallenge(ownerUserId: number, input: { releaseId: number; idempotencyKey: string; executionMode?: 'mocked' | 'live' }, repository: ManagedSiteLiveConnectorRepository = getManagedSiteLiveConnectorRepository(), clock: () => Date = () => new Date(), adapter?: ManagedSiteExistingSiteOwnershipAdapter) {
  ...
  const providerChallenge = adapter ? await adapter.createChallenge({ ownerUserId, projectId: release.projectId, releaseId: release.id, canonicalDomain: release.canonicalDomain, verificationMethod: 'dns_txt', providerAuthority, requestFingerprint, idempotencyKey: input.idempotencyKey, timeoutMs: DEPLOYMENT_TIMEOUT_MS }) : null
  if (providerChallenge) assertManagedSiteProviderAuthorityFingerprint(providerChallenge.providerAuthorityFingerprint, providerAuthority)
...
export async function verifyExistingSiteOwnership(ownerUserId: number, input: { releaseId: number; challengeReceiptFingerprint: string; executionMode: 'mocked' | 'live'; idempotencyKey: string }, adapter: ManagedSiteExistingSiteOwnershipAdapter, dependencies: { repository?: ManagedSiteLiveConnectorRepository; credentialResolver?: ManagedSiteCredentialResolver; clock?: () => Date } = {}) {
  ...
  const challengeAuthority = challenge.metadata as Record<string, unknown>
  if (challengeAuthority.providerAuthorityFingerprint !== providerAuthority.authorityFingerprint || challengeAuthority.configurationFingerprint !== providerAuthority.configurationFingerprint || challengeAuthority.verificationReceiptFingerprint !== providerAuthority.verificationReceiptFingerprint || challengeAuthority.capabilityIdentity !== providerAuthority.capabilityIdentity) conflict('Ownership provider configuration changed after challenge; create a new governed challenge.')
  const challengeReference = challenge.externalReference
  const requestFingerprint = stableFingerprint({ ownerUserId, projectId: release.projectId, releaseId: release.id, canonicalDomain: release.canonicalDomain, challengeReceiptFingerprint: challenge.receiptFingerprint, challengeReference, providerAuthorityFingerprint: providerAuthority.authorityFingerprint })
...
export async function rollbackManagedSiteRelease(ownerUserId: number, input: { fromReleaseId: number; toReleaseId: number; executionMode: 'mocked' | 'live'; idempotencyKey: string }, adapter: ManagedSiteDeploymentAdapter, dependencies: { repository?: ManagedSiteLiveConnectorRepository; credentialResolver?: ManagedSiteCredentialResolver; clock?: () => Date } = {}) {
  ...
  const priorAuthority = priorReceipt.metadata as Record<string, unknown>
  if (priorAuthority.providerAuthorityFingerprint !== providerAuthority.authorityFingerprint || priorAuthority.configurationFingerprint !== providerAuthority.configurationFingerprint || priorAuthority.verificationReceiptFingerprint !== providerAuthority.verificationReceiptFingerprint || priorAuthority.capabilityIdentity !== providerAuthority.capabilityIdentity) conflict('Rollback provider configuration differs from the verified target deployment authority.')
  const requestFingerprint = stableFingerprint({ operation: 'rollback', fromReleaseId: from.id, toReleaseId: to.id, priorDeploymentReceiptFingerprint: priorReceipt.receiptFingerprint, targetContentHash: to.contentHash, providerAuthorityFingerprint: providerAuthority.authorityFingerprint })
  ...
  try {
    const result = await adapter.rollback({ projectId: from.projectId, fromReleaseId: from.id, toReleaseId: to.id, versionId: to.versionId, contentHash: to.contentHash, canonicalDomain: to.canonicalDomain, priorDeploymentReceiptFingerprint: priorReceipt.receiptFingerprint, providerAuthority, requestFingerprint, timeoutMs: DEPLOYMENT_TIMEOUT_MS })
    validateDeploymentReceipt(result, { providerKey: providerAuthority.providerKey, providerAuthorityFingerprint: providerAuthority.authorityFingerprint, projectId: to.projectId, versionId: to.versionId, contentHash: to.contentHash, canonicalDomain: to.canonicalDomain, status: 'rollback_verified' })
    ...
    const { target, receipt } = await repository.transaction(async transaction => {
      const receipt = await transaction.insertReceipt({ ownerUserId, projectId: to.projectId, draftOrderId: priorReceipt.draftOrderId, releaseId: to.id, attemptId: leased.id, capability: 'deployment', providerKey: result.providerKey, providerEventId: result.providerEventId, receiptType: 'rollback_deployment_verified', receiptStatus: 'verified', externalReference: result.providerDeploymentId, exactResponseIdentity: result.exactResponseIdentity, requestFingerprint, contentHash: to.contentHash, canonicalDomain: to.canonicalDomain, metadata: { fromReleaseId: from.id, toReleaseId: to.id, priorDeploymentReceiptFingerprint: priorReceipt.receiptFingerprint, ...managedSiteProviderAuthorityMetadata(providerAuthority) }, receiptFingerprint, verifiedAt: clock() } as any)
...
export function createMockManagedSiteDeploymentAdapter(options: { providerKey?: string; failOperations?: Partial<Record<'preview' | 'deploy' | 'rollback', number>>; collision?: Partial<ManagedSiteDeploymentReceipt>; now?: () => Date } = {}): ManagedSiteDeploymentAdapter {
  ...
  const receipt = (input: { projectId: number; versionId: number; contentHash: string; canonicalDomain: string; requestFingerprint: string; providerAuthority: import('./types').ManagedSiteProviderAuthoritySnapshot }, status: ManagedSiteDeploymentReceipt['status']): ManagedSiteDeploymentReceipt => { const core = { providerKey, providerEventId: `${status}-${stableFingerprint(input).slice(0, 24)}`, providerDeploymentId: `deployment-${stableFingerprint({ input, status }).slice(0, 24)}`, projectId: input.projectId, versionId: input.versionId, contentHash: input.contentHash, canonicalDomain: input.canonicalDomain, deploymentUrl: status === 'preview_ready' ? `https://preview-${stableFingerprint(input).slice(0, 16)}.preview.discoverystack.dev` : `https://${input.canonicalDomain}`, status, observedAt: (options.now || (() => new Date('2026-08-27T00:00:00.000Z')))().toISOString(), providerAuthorityFingerprint: input.providerAuthority.authorityFingerprint }; return { ...core, payloadHash: stableFingerprint(core), exactResponseIdentity: `deployment-response:${stableFingerprint({ input, status }).slice(0, 32)}`, ...options.collision } }
  return {
