...
export async function generateManagedSiteCandidate(
  ...
  try {
    ...
    const admitted = admitManagedSiteGenerationOutput(output, { requestFingerprint: request.requestFingerprint, providerKey })
    ...
    const candidate = await repository.transaction(async transaction => {
      const created = await transaction.insertGenerationCandidate({ ownerUserId, projectId: project.id, sourceVersionId: version.id, requestSchemaVersion: request.schemaVersion, requestFingerprint: request.requestFingerprint, idempotencyKey: input.idempotencyKey, providerKey: admitted.output.providerKey, providerModel: admitted.output.providerModel, providerRequestId: admitted.output.providerRequestId, manifest: admitted.manifest, manifestHash: admitted.manifest.manifestHash, contentHash: admitted.manifest.contentHash, vaultReference: stored.vaultReference, gateSummary: { schema: 'passed', paths: 'passed', size: 'passed', contentSafety: 'passed', deterministicManifest: 'passed', humanReview: 'required', previewBuild: 'not_run', securityGate: 'not_run' } } as any)
