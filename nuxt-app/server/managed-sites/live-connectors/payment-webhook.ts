...
function receiptOccurredAt(receipt: ManagedSiteConnectorReceipt): number {
  const occurredAt = plainRecord(receipt.metadata)?.occurredAt
  if (typeof occurredAt !== 'string') return Number.NEGATIVE_INFINITY
  const parsed = new Date(occurredAt).getTime()
  return Number.isFinite(parsed) ? parsed : Number.NEGATIVE_INFINITY
}
function latestIgnoredSettlement(receipts: ManagedSiteConnectorReceipt[], providerKey: string): ManagedSiteConnectorReceipt | null {
  const ignored = receipts.filter(receipt => ['payment_refunded', 'payment_disputed'].includes(receipt.receiptType) && receipt.receiptStatus === 'ignored_out_of_order' && receipt.providerKey === providerKey)
  const disputes = ignored.filter(receipt => receipt.receiptType === 'payment_disputed')
  const candidates = disputes.length ? disputes : ignored.filter(receipt => receipt.receiptType === 'payment_refunded')
  return candidates.reduce<ManagedSiteConnectorReceipt | null>((latest, candidate) => {
    if (!latest) return candidate
    const latestOccurredAt = receiptOccurredAt(latest); const candidateOccurredAt = receiptOccurredAt(candidate)
    return candidateOccurredAt > latestOccurredAt || candidateOccurredAt === latestOccurredAt && candidate.id > latest.id ? candidate : latest
  }, null)
}
...
export async function processManagedSiteVerifiedPaymentWebhook(
  ...
  return transact(async repositories => {
    ...
    let effective = true; let receiptStatus: 'verified' | 'ignored_out_of_order' = 'verified'; let settledBy: ManagedSiteConnectorReceipt | null = null; const projectId = order.projectId
    if (event.eventType === 'checkout_succeeded') {
      if (previousSuccess || previousCancel) { effective = false; receiptStatus = 'ignored_out_of_order' }
        ...
        settledBy = latestIgnoredSettlement(receipts, event.providerKey)
        ...
        if (settledBy) {
          await repositories.ordering.updateDraftOrder(order.id, { status: settledBy.receiptType === 'payment_disputed' ? 'disputed' : 'refunded', updatedAt: clock() } as any)
