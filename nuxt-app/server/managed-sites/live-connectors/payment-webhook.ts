...
function validateVerifiedEvent(event: ManagedSiteVerifiedPaymentWebhook, clock: () => Date): ManagedSiteVerifiedPaymentWebhook {
  ...
  if (!Number.isSafeInteger(event.draftOrderId) || event.draftOrderId < 1 || !Number.isSafeInteger(event.amountMinor) || event.amountMinor < 0 || !/^[A-Z]{3}$/u.test(event.currency) || ![event.canonicalPayloadHash, event.configurationFingerprint, event.verificationReceiptFingerprint, event.checkoutReceiptFingerprint].every(value => /^[a-f0-9]{64}$/u.test(value))) invalid('Verified payment webhook commercial or provider configuration identity is invalid.')
