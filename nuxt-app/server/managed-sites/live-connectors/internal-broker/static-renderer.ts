...
function renderPage(blueprint: ManagedSiteBlueprintV1, page: ManagedSiteBlueprintV1['pages'][number]): string {
  ...
  const sections = page.sections.map(section => `<section id="${escapeHtml(section.sectionId)}"><h2>${escapeHtml(section.heading)}</h2><p>${escapeHtml(section.body)}</p>${section.ctaLabel && section.ctaHref ? `<a class="cta" href="${safeHref(section.ctaHref)}">${escapeHtml(section.ctaLabel)}</a>` : ''}${section.moduleKey ? `<p data-module-placeholder="${escapeHtml(section.moduleKey)}">${escapeHtml(section.moduleKey)}</p>` : ''}</section>`).join('')
