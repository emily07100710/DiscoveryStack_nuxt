...
export type MediaAssetStatus =
  ...
  | "deleted"
  | "failed";
export type MediaScannerVerdict =
  | "passed"
  | "blocked"
  | "owner_review"
  | "not_configured";

export interface MediaTenantScope {
  ownerUserId: number;
  projectId: number;
}
export interface MediaActor extends MediaTenantScope {
  ...
  authority:
    ...
    | "system_workflow"
    | "system_test";
  role: "platform_owner" | "customer_admin" | "editor" | "viewer";
}
export interface MediaQuota {
  maxOriginalBytes: number;
  maxAssetCount: number;
  maxMonthlyUploadBytes: number;
  maxMonthlyProcessingCount: number;
  originalBytesUsed: number;
  assetCountUsed: number;
  monthlyUploadBytesUsed: number;
  monthlyProcessingCountUsed: number;
  periodKey: string;
}
export interface MediaQuotaDelta {
  originalBytes: number;
  assetCount: number;
  uploadBytes: number;
  processingCount: number;
}
export interface MediaQuotaClaim {
  idempotencyKey: string;
  requestFingerprint: string;
  periodKey: string;
  delta: MediaQuotaDelta;
...
export interface MediaSignedAuthorization {
  method: "PUT" | "GET";
  url: string;
  expiresAt: string;
...
export interface MediaStoragePort {
  ...
  ): Promise<MediaSignedAuthorization>;
  completeUpload(
    input: MediaTenantScope & {
      uploadId: string;
      objectKey: string;
  ...
  headObject(
    input: MediaTenantScope & { objectKey: string }
  ): Promise<MediaObjectHead | null>;
  readForProcessing(
    input: MediaTenantScope & { objectKey: string; maxBytes: number }
  ): Promise<Uint8Array>;
  writeVariant(
    input: MediaTenantScope & {
      objectKey: string;
  ...
  createSignedRead(
    input: MediaTenantScope & { objectKey: string; expiresAt: Date }
  ): Promise<MediaSignedAuthorization>;
  deleteObject(
    input: MediaTenantScope & { objectKey: string }
  ): Promise<{ deleted: boolean; receiptReference: string }>;
...
export interface MediaInspection {
  mime: (typeof MEDIA_MIME_ALLOWLIST)[number];
  width: number;
  height: number;
  frameCount: number;
...
export interface MediaSecurityScanner {
  scan(input: {
    bytes: Uint8Array;
    sha256: string;
    tenant: MediaTenantScope;
  }): Promise<{
    verdict: MediaScannerVerdict;
...
export interface MediaVariantOutput {
  key: (typeof MEDIA_VARIANT_KEYS)[number];
...
export interface MediaTransformation {
  crop?: {
    ...
    width: number;
    height: number;
    aspect: "free" | "1:1" | "4:3" | "3:2" | "16:9" | "portrait";
  };
...
export interface MediaImageProcessor {
  inspect(bytes: Uint8Array): Promise<MediaInspection>;
  produceVariants(input: {
    bytes: Uint8Array;
...
export interface MediaAssetProjection extends MediaTenantScope {
  ...
  scannerVerdict: MediaScannerVerdict | null;
  variants: MediaVariantProjection[];
  collectionId: number | null;
  tags: Array<{ id: number; name: string; canonicalKey: string }>;
  rightsMetadata: MediaRightsMetadata;
  createdAt: string;
  trashedAt: string | null;
  retentionUntil: string | null;
  deletedAt: string | null;
}
export interface MediaVariantProjection {
  key: string;
  format: string;
  width: number;
  height: number;
...
export interface MediaAssetPageOptions {
  limit?: number;
...
export interface MediaVaultRepository {
  ...
  ): Promise<MediaQuotaClaim>;
  settleQuota(
    scope: MediaTenantScope,
    input: {
      idempotencyKey: string;
      requestFingerprint: string;
      committed: MediaQuotaDelta;
    }
  ): Promise<MediaQuotaClaim>;
  releaseQuota(
    scope: MediaTenantScope,
  ...
  creditQuota(
    ...
    input: {
      idempotencyKey: string;
      requestFingerprint: string;
      delta: Pick<MediaQuotaDelta, "originalBytes" | "assetCount">;
    }
  ): Promise<MediaQuotaClaim>;
  ...
  updateUpload(
    scope: MediaTenantScope,
    uploadId: string,
    patch: Partial<MediaUploadSession>
  ): Promise<MediaUploadSession | null>;
  claimUploadStatus(
    scope: MediaTenantScope,
    uploadId: string,
    from: MediaUploadSession["status"][],
    to: MediaUploadSession["status"]
  ): Promise<boolean>;
  findAsset(
    scope: MediaTenantScope,
    assetId: string
  ): Promise<MediaAssetProjection | null>;
  findReadyAssetByHash(
    scope: MediaTenantScope,
    sha256: string
  ): Promise<MediaAssetProjection | null>;
  listAssets(
  ...
  updateAsset(
    scope: MediaTenantScope,
    assetId: string,
    patch: Partial<MediaAssetProjection>
  ): Promise<MediaAssetProjection | null>;
  appendReplacementVersion(
    scope: MediaTenantScope,
    replacement: MediaAssetProjection
  ): Promise<MediaAssetProjection | null>;
  claimAssetStatus(
    scope: MediaTenantScope,
    assetId: string,
  ...
  recordProcessingRun(
    ...
    input: {
      assetId: string;
      status: "quarantined" | "failed" | "succeeded";
      scannerVerdict: MediaScannerVerdict;
  ...
  findEventByIdempotency(
    scope: MediaTenantScope,
    input: { assetId: string; eventType: string; idempotencyKey: string }
