...
export function createS3MediaStorage(options: { configuration: S3MediaConfiguration; client?: Pick<S3Client, 'send'>; presign?: typeof getSignedUrl }): MediaStoragePort {
  ...
  return {
    kind: 's3_compatible',
    async createUploadIntent(input) { const expiresIn = safeExpiry(input.expiresAt); const key = configuredKey(configuration, input, input.objectKey); const command = new PutObjectCommand({ Bucket: configuration.bucket, Key: key, ContentType: input.contentType, ContentLength: input.byteSize, Metadata: { uploadid: input.uploadId, ownerid: String(input.ownerUserId), projectid: String(input.projectId) } }); const url = await presign(client as S3Client, command, { expiresIn }); return { method: 'PUT', url, expiresAt: input.expiresAt.toISOString(), requiredHeaders: { 'content-type': input.contentType, 'content-length': String(input.byteSize), 'x-amz-meta-uploadid': input.uploadId, 'x-amz-meta-ownerid': String(input.ownerUserId), 'x-amz-meta-projectid': String(input.projectId) }, cacheControl: 'private, no-store' } },
