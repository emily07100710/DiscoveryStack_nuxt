...
export function createMemoryMediaStorage(): MemoryStorage {
  ...
  return {
    ...
    async createUploadIntent(input) { assertObjectKey(input, input.objectKey); return { method: 'PUT', url: `memory-upload://${input.uploadId}`, expiresAt: input.expiresAt.toISOString(), requiredHeaders: { 'content-type': input.contentType, 'content-length': String(input.byteSize) }, cacheControl: 'private, no-store' } },
    async completeUpload(input) { const value = objects.get(identity(input, input.objectKey)); if (!value) throw createError({ statusCode: 409, statusMessage: 'Uploaded media object is missing.' }); return { byteSize: value.bytes.byteLength, contentType: value.contentType, etag: sha256(value.bytes), versionReference: value.version, metadata: {} } },
...
function validateS3Configuration(input: S3MediaConfiguration): S3MediaConfiguration {
  ...
  for (const raw of [input.endpoint, input.publicCdnOrigin].filter(Boolean) as string[]) {
    let url: URL; try { url = new URL(raw) } catch { throw createError({ statusCode: 503, statusMessage: 'Media storage origin is invalid.' }) }
    if (url.protocol !== 'https:' || url.username || url.password || url.pathname !== '/' || url.search || url.hash || url.port && url.port !== '443' || isIP(url.hostname) || url.hostname === 'localhost' || url.hostname.endsWith('.local')) throw createError({ statusCode: 503, statusMessage: 'Media storage origin is not an approved HTTPS host.' })
  }
...
export function createS3MediaStorage(options: { configuration: S3MediaConfiguration; client?: Pick<S3Client, 'send'>; presign?: typeof getSignedUrl }): MediaStoragePort {
  ...
  const rawCredential = process.env[configuration.credentialReference]
  ...
  return {
    kind: 's3_compatible',
    async createUploadIntent(input) { const expiresIn = safeExpiry(input.expiresAt); const key = configuredKey(configuration, input, input.objectKey); const command = new PutObjectCommand({ Bucket: configuration.bucket, Key: key, ContentType: input.contentType, ContentLength: input.byteSize, Metadata: { uploadid: input.uploadId, ownerid: String(input.ownerUserId), projectid: String(input.projectId) } }); const url = await presign(client as S3Client, command, { expiresIn }); return { method: 'PUT', url, expiresAt: input.expiresAt.toISOString(), requiredHeaders: { 'content-type': input.contentType, 'content-length': String(input.byteSize) }, cacheControl: 'private, no-store' } },
    ...
    async headObject(input) { try { return await this.completeUpload(input) } catch (error: any) { if (error?.name === 'NotFound' || error?.name === 'NoSuchKey' || error?.$metadata?.httpStatusCode === 404) return null; throw createError({ statusCode: 503, statusMessage: 'Media storage lookup failed.' }) } },
    ...
    async healthCheck() { return { ready: true, mode: 's3-compatible-configured' } },
  }
}
