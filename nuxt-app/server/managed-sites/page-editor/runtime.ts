...
function resolveMediaScanner(): MediaSecurityScanner { const endpoint = process.env.NUXT_MEDIA_SCANNER_ENDPOINT; const credentialReference = process.env.NUXT_MEDIA_SCANNER_CREDENTIAL_REF; if (!endpoint || !credentialReference) return createUnavailableProductionScanner('quarantine'); return createHttpMediaSecurityScanner({ endpoint, credentialReference }) }
export async function resolveEditorRuntime(scope: MediaTenantScope) { const storage = await resolveMediaStorage(scope); let processor: MediaImageProcessor = unavailableProcessor; try { processor = createSharpMediaImageProcessor() } catch { /* resolver remains fail closed when native codec is unavailable */ } return { mediaRepository: getDrizzleMediaVaultRepository(), pageRepository: getDrizzlePageEditorRepository(), storage, scanner: resolveMediaScanner(), processor } }
export async function resolvePagePublicationAuthority(scope: MediaTenantScope) {
  ...
  const [target] = await database.select({ id: contentOperationPublicationTargets.id }).from(contentOperationPublicationTargets).where(and(eq(contentOperationPublicationTargets.ownerUserId, scope.ownerUserId), eq(contentOperationPublicationTargets.clientId, project.clientId), eq(contentOperationPublicationTargets.status, 'active'))).orderBy(desc(contentOperationPublicationTargets.updatedAt)).limit(1); if (!target) throw createError({ statusCode: 409, statusMessage: 'No active first-party publication target is configured for this managed site.' })
  return { releaseReference: `release:${release.id}`, publicationTargetReference: `target:${target.id}` }
}
