...
export function createMemoryPageEditorRepository(): PageEditorRepository {
  const pages = new Map<string, PageDocument[]>(); const operations = new Map<string, AppliedPageOperation>(); const receipts = new Map<string, PagePublicationReceipt[]>(); const publicationClaims = new Map<string, { fingerprint: string; receipt: PagePublicationReceipt; workIds: number[] }>(); let workSequence = 0
  ...
  const repository: PageEditorRepository = {
    ...
    async enqueuePublication(input) { const key = `${scope(input.ownerUserId, input.projectId)}:${input.idempotencyKey}`; const fingerprint = canonicalFingerprint({ ...input, createdAt: input.createdAt.toISOString() }); const existing = publicationClaims.get(key); if (existing) { if (existing.fingerprint !== fingerprint) throw createError({ statusCode: 409, statusMessage: 'Publication idempotency key collided with different canonical work.' }); return { receipt: clone(existing.receipt), workIds: [...existing.workIds], replayed: true } } const targetId = input.targetIds[0]; if (!targetId) throw createError({ statusCode: 422, statusMessage: 'Publication target is required.' }); const receipt: PagePublicationReceipt = { pageId: input.page.pageId, pageVersion: input.page.version, status: 'intent_created', artifactFingerprint: input.artifact.artifactFingerprint, mediaSetFingerprint: input.artifact.mediaSetFingerprint, releaseReference: `release:${input.releaseId}`, publicationTargetReference: `target:${targetId}`, receiptFingerprint: canonicalFingerprint({ version: 'managed-site-page-publication-intent-v2', fingerprint, targetId }), createdAt: input.createdAt.toISOString() }; await repository.appendPublicationReceipt(input.ownerUserId, input.projectId, receipt); const workIds = input.targetIds.map(() => ++workSequence); publicationClaims.set(key, { fingerprint, receipt: clone(receipt), workIds }); return { receipt, workIds, replayed: false } },
