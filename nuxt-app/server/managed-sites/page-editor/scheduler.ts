...
export interface EditorSchedulerJob { jobId: string; ownerUserId: number; projectId: number; kind: EditorJobKind; attempt: number; availableAt: string; leaseUntil: string | null; stateFingerprint: string }
...
export async function runEditorSchedulerTick(port: EditorSchedulerPort, handlers: EditorJobHandlers, now = new Date()) {
  ...
  for (const job of jobs) { const tenant = `${job.ownerUserId}:${job.projectId}`; const count = (tenantCounts.get(tenant) || 0) + 1; tenantCounts.set(tenant, count); if (count > EDITOR_SCHEDULER_LIMITS.maxJobsPerTenant) { await port.block(job, 'TENANT_TICK_BOUND_EXCEEDED', now); continue } try { const result = await handlers[job.kind](job); if (result.externalCalls) throw new Error('Scheduler handlers must use separately governed adapters.'); const fingerprint = canonicalFingerprint({ version: 'managed-site-editor-scheduler-receipt-v1', jobId: job.jobId, stateFingerprint: job.stateFingerprint, attempt: job.attempt, outcome: result.outcome }); await port.succeed(job, { outcome: result.outcome, fingerprint }, now); receipts.push({ jobId: job.jobId, status: 'succeeded', fingerprint }) } catch (error: any) { const reasonCode = typeof error?.reasonCode === 'string' ? error.reasonCode : 'EDITOR_JOB_FAILED'; if (job.attempt >= EDITOR_SCHEDULER_LIMITS.maxAttempts) { await port.block(job, reasonCode, now); receipts.push({ jobId: job.jobId, status: 'blocked', reasonCode }) } else { const nextAttemptAt = new Date(now.getTime() + Math.min(60 * 60 * 1000, 30_000 * 2 ** Math.max(0, job.attempt - 1))); await port.retry(job, { reasonCode, nextAttemptAt }, now); receipts.push({ jobId: job.jobId, status: 'retry_wait', reasonCode, nextAttemptAt: nextAttemptAt.toISOString() }) } } }
  return { version: 'managed-site-editor-scheduler-tick-v1', claimed: jobs.length, tenants: tenantCounts.size, receipts, bounded: true, externalCalls: false }
}
