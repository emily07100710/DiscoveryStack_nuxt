...
ALTER TABLE `seoGeoDeliveryAttempts` ADD CONSTRAINT `seoGeoDeliveryAttempts_approvalReviewId_seoGeoContentReviews_id_fk` FOREIGN KEY (`approvalReviewId`) REFERENCES `seoGeoContentReviews`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `seoGeoEvidenceApprovals` ADD CONSTRAINT `seoGeoEvidenceApprovals_artifactId_publicIntelligenceArtifacts_id_fk` FOREIGN KEY (`artifactId`) REFERENCES `publicIntelligenceArtifacts`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
