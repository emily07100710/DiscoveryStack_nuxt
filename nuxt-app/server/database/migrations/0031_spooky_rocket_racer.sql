...
ALTER TABLE `systemProvisioningPlans` ADD CONSTRAINT `spp_spec_version_fk` FOREIGN KEY (`systemSpecVersionId`) REFERENCES `systemSpecVersions`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `systemTenantBindings` ADD CONSTRAINT `stb_managed_project_fk` FOREIGN KEY (`managedSiteProjectId`) REFERENCES `managedSiteProjects`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `systemTenantBindings` ADD CONSTRAINT `stb_managed_preview_fk` FOREIGN KEY (`managedSitePreviewId`) REFERENCES `managedSitePreviews`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `systemTenantBindings` ADD CONSTRAINT `stb_draft_order_fk` FOREIGN KEY (`managedSiteDraftOrderId`) REFERENCES `managedSiteDraftOrders`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `systemTenantBindings` ADD CONSTRAINT `stb_payment_event_fk` FOREIGN KEY (`managedSitePaymentEventId`) REFERENCES `managedSitePaymentEvents`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
