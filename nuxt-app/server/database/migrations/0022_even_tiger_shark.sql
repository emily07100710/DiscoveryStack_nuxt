...
ALTER TABLE `managedSitePaymentEvents` ADD CONSTRAINT `managedSitePaymentEvents_draftOrderId_managedSiteDraftOrders_id_fk` FOREIGN KEY (`draftOrderId`) REFERENCES `managedSiteDraftOrders`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `managedSiteSubscriptionIntents` ADD CONSTRAINT `managedSiteSubscriptionIntents_projectId_managedSiteProjects_id_fk` FOREIGN KEY (`projectId`) REFERENCES `managedSiteProjects`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
