...
import { computed, defineEmits, defineProps, onBeforeUnmount, reactive, ref } from 'vue'
...
const copy = computed(() => isZh.value ? {
  ...
  modelImprovementNote: '後台可能定期重查公開首頁；不保存原始 HTML，也不包含姓名、Email、電話、預算或自由填寫內容。可隨時要求撤回。',
  ...
  modelImprovementNote: 'The private pipeline may periodically recheck the public homepage. It retains no raw HTML and excludes your name, email, phone, budget and free-form enquiry text. You may withdraw later.',
...
async function startAnalysis() {
  ...
  try {
    const [result] = await Promise.all([
      publicApiFetch<SiteAnalysis>('/api/site-analysis', { body: { url: website.value } }),
...
async function unlockReport() {
  ...
  try {
    ...
    await publicApiFetch('/api/leads', { body: { name: lead.name, email: lead.email, company: lead.company, website: website.value, packageInterest: 'discover', language: props.locale, message: `Industry: ${lead.industry}\nRole: ${lead.role}\nPhone: ${lead.phone}\nBudget: ${lead.budget}\nTimeline: ${lead.timeline}\n${scoreSummary}\nSource: free-analysis`, privacyConsent: lead.privacyConsent, recontactConsent: lead.recontactConsent, modelImprovementConsent: lead.modelImprovementConsent, companyFax: lead.companyFax } })
    status.value = 'report'
