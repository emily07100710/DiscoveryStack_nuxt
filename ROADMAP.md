...
## 進度
- 目標:三條線同時開工(broker 開站/爬蟲證據/知識底座),各一個對話在做,檔案互不相撞
- 🔵 進行中(對話 1):一鍵建站「真的開出一站」— broker 仲介服務第一段;做完貼完成報告回來驗收
- 🔵 進行中(對話 2):整站爬蟲/頁面證據 vertical slice — URL 清單、raw vs rendered、canonical/sitemap 稽核;發包單已交
- 🔵 進行中(對話 3):Entity + Claim + Source 知識底座 — 護城河核心,順路接孤兒 JSON-LD 引擎;發包單已交
