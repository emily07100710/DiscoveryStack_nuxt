...
## 進度
- 目標:等 broker(對話 1)與知識底座(對話 3)交件驗收;爬蟲證據已第一個落地
- 🔵 進行中(對話 1):一鍵建站「真的開出一站」— broker 仲介服務第一段;做完貼完成報告回來驗收
- 🔵 進行中(對話 3):Entity + Claim + Source 知識底座 — 護城河核心,順路接孤兒 JSON-LD 引擎;四個接線問題已由 PM 拍板回覆
...
- 另外掛著:site-evidence 新資料表(migration 0034)還沒套用到正式資料庫,線上要用整站爬檢前先批准套用;換掉臨時登入密碼(正式前,直接在 Render 改環境變數);Render 免費方案會睡 → 排程睡著時不會跑;正式網域還沒買;5 個孤兒引擎逐一給歸屬;root db:push 死 script;CI 的 NUXT_BUILD_TYPECHECK=false
