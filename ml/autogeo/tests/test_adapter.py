...
class AdapterTests(unittest.TestCase):
    ...
    def test_example_manifest_contains_the_complete_pinned_inventory(self) -> None:
        result = run(str(ROOT / "verify_assets.py"), "--manifest", str(ROOT / "asset-manifest.example.json"))
    ...
    def test_incomplete_manifest_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            completed = subprocess.run(
                [sys.executable, str(ROOT / "verify_assets.py"), "--manifest", str(manifest)],
                capture_output=True,
                text=True,
            )
            self.assertNotEqual(completed.returncode, 0)
            ...
            self.assertTrue(any("missing required assets" in error for error in result["errors"]))
