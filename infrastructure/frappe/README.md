...
The lock retains the former official image only as a base-image provenance record. It is explicitly not runtime authority because its embedded Frappe revision differs from the reviewed source pin. The project-owned build recipe pins Frappe 16.32.0, ERPNext 16.33.0 and DiscoveryStack 0.1.0. The current local image was rebuilt from those pins, and its installed source markers, custom-app tree hash, manifest digest and config digest were verified. This is local verification only: `productionApproved` remains false and live execution must fail closed until a separately reviewed production image is published and approved.
